...
|---|---|
| Asset-type routing | Map a request to the documented PixelLab route per asset type: characters, animations, sprites, top-down and platformer tilesets, isometric tiles, map objects, item and skill icons, UI, backgrounds, portraits, bitmap fonts, and effects. Name the cases with no public route — whole maps, editor-only utilities — instead of inventing one. |
