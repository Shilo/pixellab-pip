...
[![VirusTotal](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fclawhub.ai%2Fapi%2Fv1%2Fskills%2Fpixellab-pip%2Fversions%2F0.6.0&query=%24.version.security.scanners.vt.normalizedStatus&label=VirusTotal&color=2ea44f&logo=virustotal&logoColor=white&cacheSeconds=3600)](https://www.virustotal.com/gui/file-analysis/NzRlNzhlZWE4M2M3MTg5N2U3NzA2OWZmYWNkYWI0MTI6MTc4MzI0ODQxNQ==/detection)
[![Build Provenance attested](https://img.shields.io/badge/Build_Provenance-attested-2ea44f)](https://github.com/Shilo/pixellab-pip/attestations/33930120)
...
|---|---|---|
| **PixelLab Pip skill** | **~98%** | ~7.5k tokens (+ references on demand) |
| Official `mcp/docs` injected | ~58% | ~7.7k tokens (all, always) |
| No skill (agent knowledge only) | ~26% | 0 |

Across 20 scenarios phrased as **plain human requests** (`claude` + `codex` + `deepseek-v4-pro`, 1 rep) — no tool names or "which endpoint" hints, so the only difference between arms is what that user would actually have: the skill, the docs link's content, or nothing — each agent *plans* the route and is scored only when it identifies the correct PixelLab route (no partial credit for a plausible-sounding plan that names the wrong tool). The skill routed **every** scenario correctly across all three agents (the only dropped sub-check is the exact credit *figure* on one cost question); the official docs got ~58% — strong on pure MCP tool-selection but **0** on REST-only routes (backgrounds, image-pro sheets, the skeleton flow), local post-processing, setup, and blueprint replay; no skill was mostly lost. The skill injects about the same up-front context as the docs (~7.5k vs ~7.7k) and reads a reference only when a task needs it. The benchmark is **dry — it spends no PixelLab credits**. A dated, nondeterministic snapshot; see the full per-scenario tables and reproduce steps in the **[full benchmark report ↗](docs/pixellab-pip-benchmark.md)**.
