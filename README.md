...
|---|---|
| PixelLab asset coverage | Knows the documented route for each PixelLab asset type: characters, animations, sprites, top-down and platformer tilesets, isometric tiles, map objects, item and skill icons, UI, backgrounds, portraits, bitmap fonts, and effects. |
...
| Asset integrity | Requested art originates from PixelLab or from you. Local tools freely crop, resize, mask, composite, assemble, package, and palette-convert those pixels, and report that they did. Locally drawn or repaired content needs your explicit approval and is labeled as non-PixelLab, so a failed generation is never quietly patched into a claimed final asset. Outputs are checked against your stated constraints, and a failure is reported rather than salvaged. |
| Local output folder | Downloads, previews, blueprints, manifests, and packages are saved under a project `pixellab-pip-generations/` folder. When a job returns separate frames, Pip also compiles a single spritesheet. |
...
| Preset and custom animation | Maps motion wording to PixelLab's template animation IDs (walk, run, idle, jump, and more, per body type — humanoid, dog, cat, horse, bear, lion), or routes custom described actions, auto-rigging from an estimated skeleton, and start-to-end tweens. One direction is previewed before animating all eight, and PixelLab's returned frame order is preserved. |
