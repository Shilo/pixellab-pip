...
|---|---|
| Easy PixelLab workflow | Ask for assets in plain language — no tool names, settings, or modes to learn. Pip works out which PixelLab tool fits each kind of asset (characters, animations, sprites, top-down and platformer tilesets, isometric tiles, map objects, icons, UI, backgrounds, portraits, fonts, effects), and whether to use PixelLab's MCP tools, its API, the website editor, or Aseprite. When PixelLab has no supported way to do something, it says so instead of inventing one. |
| Guided PixelLab setup | Connect PixelLab to your assistant or editor, work out what is missing when generation fails, and change only what you approve — without ever reading or printing your secret. |
| Cost and credit control | Say cheap or set a budget and Pip picks the cheapest option that fits, names the tradeoff, and does not quietly upgrade you to a pricier Pro one. It lists what it plans to make and the cost before a batch, asks before each extra paid attempt, and re-checks a stuck job instead of paying for it twice. |
| Asset integrity | Your art comes from PixelLab or from you. Pip may crop, mask, assemble, package, and convert those pixels, and says when it did; it will not resize, combine, or patch a result and call it final without your approval, and anything it draws itself is labeled as not from PixelLab. Results are checked against what you asked for, and a failure is reported rather than papered over. |
| Local output folder | Downloads, previews, blueprints, packages, and a record of each job are saved in a `pixellab-pip-generations/` folder in your project unless you name another location. When a job comes back as separate frames, Pip also builds a single spritesheet. |
| Generation reports | Every generation is reported: which PixelLab tool ran, the exact wording sent, the settings that mattered, where the files went, what it cost, and whether it passed the checks — plus a record saved beside the files with the job IDs and seed. |
| Preset and custom animation | Describe an action and it goes to PixelLab's text animation, its recommended default. Name a stock motion (walk, run, idle, jump, and more — for humanoids, dogs, cats, horses, bears, and lions) and Pip picks the matching PixelLab preset. It can also rig a sprite from its skeleton when you want control over the joints. One direction is previewed before animating the rest, and frames stay in the order PixelLab returned them. |
...
| Images you attach | Pip works out what each file is for — the image to edit, a character to match, a style to copy, a color palette, a mask, or the first frame of an animation — and asks first when guessing wrong would cost you credits. |
...
| Prompt preparation | Turn rough wording, your images, styles, palettes, and constraints into a clear PixelLab-ready description, using PixelLab's own prompt enhancer when the tool has one. Anything it adds is shown before you pay, and wording you give exactly is sent untouched. Opt out any time. |
| Reusable Blueprints | Save a workflow as a small, shareable blueprint holding the exact PixelLab requests plus the steps around them. Recreate it anytime with plain-language changes, or run the bundled blueprints by name — a replay repeats the workflow and inputs, not the exact pixels, and spends credits again. Full guide: [docs/blueprint.md](docs/blueprint.md). |
| Aseprite handoff | Move generated assets into Aseprite through its own command line rather than the PixelLab extension: `.aseprite` copies that leave your original untouched, imports and exports that keep frame order, and palette clamps (1-bit, Game Boy green, a palette Aseprite has installed, or your own hex colors) checked against the colors you asked for. |
| Fallback background removal | When you asked for a transparent asset and it comes back with a background, Pip checks whether it can be cleaned up locally without touching the art, and hands it to PixelLab's own background removal when unsure. |
| Paperdoll and layered characters | Fitted hair, armor, hats, weapons, and accessories on an existing character, delivered as the base, one transparent PNG per layer, and the finished composite, or as Aseprite layers. Each layer is checked to hold only the new part, without pretending PixelLab returns layers where it does not. |
| Answer PixelLab questions | Explain setup, sign-in, docs, code libraries, troubleshooting, and confusing labels such as `Pro`, `v3`, `new`, `create tiles` vs `create tileset`, Pixen, PixFlux, BitForge, and PixPatch. Official docs are re-checked when a needed fact is missing or unclear, and anything it cannot verify is flagged. |
| Non-English request support | Send PixelLab the English it needs while keeping questions, explanations, and reports in your language, and show the English it actually sent. |
| Token and secret handling | Set up your PixelLab token without exposing its value, dumping environment variables, or scanning `.env*` files, shell history, or keychains. No copied website session tokens, and none of the private endpoints PixelLab's own website and Aseprite extension use behind the scenes. |
