...
|---|---|
| Easy PixelLab workflow | Ask for assets in plain language without knowing PixelLab tool names, endpoint names, modes, or editor differences. Pip maps the request to the documented route for each asset type — characters, animations, sprites, top-down and platformer tilesets, isometric tiles, map objects, item and skill icons, UI, backgrounds, portraits, bitmap fonts, effects — picking between MCP tools, REST v2 for the routes MCP does not cover, the website editor, and Aseprite, then reports which it used. Where PixelLab has no public route, such as whole maps and editor-only utilities, it says so instead of inventing one. |
...
| Cost and credit control | Say cheap or set a budget and Pip prefers the lowest-cost route that fits, names the tradeoff, and does not silently upgrade to Pro. It lists planned items with cost category before a batch, asks before each extra paid attempt unless you approved a budget or attempt count, and re-checks a stalled job instead of resubmitting a paid one. |
| Asset integrity | Requested art originates from PixelLab or from you. Local tools crop, mask, assemble, package, and format-convert those pixels, and report that they did. Resizing, compositing, reassembling, or repairing an output locally is not called final without your approval, and locally drawn content is labeled non-PixelLab, so a failed generation is never quietly patched into a claimed final asset. Outputs are checked against your stated constraints, and a failure is reported rather than salvaged. |
...
| Generation reports | Report the PixelLab tool or endpoint used, the exact wording sent, the settings that mattered, output locations, async status, cost, and verification status, plus a manifest saved beside the outputs holding the job/asset IDs and seed. |
| Preset and custom animation | Custom described actions go to PixelLab's text animation, its recommended default. Named motion wording maps to its template animation IDs (walk, run, idle, jump, and more, per body type — humanoid, dog, cat, horse, bear, lion), and skeleton estimate/animate is used when you want to own the keypoints. One direction is previewed first; all directions are animated only if you ask, and PixelLab's returned frame order is preserved. |
| Multi-shot cinematics and seamless loops | PixelLab animates one short clip at a time. For a longer or seamlessly looping scene, Pip decides between one looped clip and chained shots, plans the beats, validates every step, and hard-stops at the budget you set — which it requires before spending. Works from a supplied frame or from scratch. |
| Image and attachment roles | Identify each supplied file's role — edit target, identity reference, style reference, concept image, mask, palette, init/source image, or animation frame — instead of guessing one generic role, and ask when a wrong guess would cost credits. |
...
| Prompt preparation | Turn rough wording, visible inputs, styles, palettes, and constraints into concise PixelLab-ready descriptions by default, using PixelLab's own enhancer when the route has one. Material additions are shown before a paid call; exact field values you supply are sent unchanged, or flagged if the endpoint would reject them. Opt out any time. |
| Reusable Blueprints | Save a PixelLab workflow as a small, shareable blueprint with exact MCP/REST requests plus flexible agent tasks for preparation, selection, assembly, and verification. Recreate it anytime with plain-language overrides, or run bundled blueprints by name — a replay reproduces the workflow and inputs, not the exact pixels, and spends credits again. Full guide: [docs/blueprint.md](docs/blueprint.md). |
| Local Aseprite CLI workspaces | Put generated assets into Aseprite through documented CLI/Lua instead of the PixelLab extension UI: `.aseprite` copies that leave your original untouched, frame-order-preserving import and export, and palette clamps — 1-bit black/white, Game Boy green, an installed palette Aseprite can load, or supplied hex colors — checked against the colors you asked for. |
...
| Answer PixelLab questions | Explain setup, auth, docs, SDKs, troubleshooting, and confusing labels such as `Pro`, `v3`, `new`, `create tiles`, `create tileset`, Pixen, PixFlux, BitForge, and PixPatch at the documented product level. Official docs, schemas, pricing, and limits are refreshed when a needed fact is missing or unclear, and anything unverifiable is flagged. |
| Non-English request support | Translate or normalize PixelLab-facing natural-language inputs to concise English while keeping confirmations, explanations, and reports in your language, with the English values actually sent shown as-is. |
| Auth and secret handling | Bearer-token and MCP secret setup without exposing token values, dumping environment variables, or scanning for `.env*` files, shell history, or keychains; an exact `.env` file is read only if you name it and approve it for troubleshooting. No copied website session tokens, and no undocumented internal endpoints used by first-party surfaces such as the website or Aseprite extension. |
