...
| Pick from generated choices | When PixelLab returns several possible results, see numbered choices that start at 1 and pick what to keep before Pip saves, edits, or continues from one. |
...
| Reusable Blueprints | Save a generation as a small, shareable blueprint file recording the exact tool/route and inputs, then recreate it anytime — your own or one someone sent you — overriding any value in plain language (new prompt, same seed, random seed). Run bundled blueprints by name, e.g. "create the knight blueprint". Full guide: [docs/blueprint.md](docs/blueprint.md). |
...
| Multi-shot cinematics and seamless loops | Describe a scene, a length, and a budget, and get a multi-second or seamlessly looping animation built shot by shot from a starting frame — Pip plans the beats, chains each shot from the last one's frame, validates every step, and stops at the budget you set. |
