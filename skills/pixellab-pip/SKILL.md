...
| Editor-only utilities such as Canny/Pose/Depth, PixelLab extension reduce colors, unzoom, pixel correction, and reshape | Use visible website/editor/Aseprite/Pixelorama workflow when the user wants exact PixelLab editor behavior. For explicit local Aseprite palette quantization, color reduction, indexed conversion, or palette replacement, read `references/aseprite-cli.md`. Otherwise use the closest documented REST route or a labeled local fallback only after explaining the difference. Read `references/editor-only-utilities.md` before giving exact route advice. | No public REST v2/MCP route was documented for the exact editor-only utility behaviors. Do not invent `/v2/...` routes or automate first-party internal endpoints. |
