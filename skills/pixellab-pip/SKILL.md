...
| Fitted paperdoll addition on an existing character image | Use an existing-image edit surface anchored on the base character frame. When the user is actively working in Aseprite and the installed extension exposes a changes-only new-layer `output_method`, request that editor output mode and verify the exported layer. For public API/code routes, use a documented REST v2 image-edit endpoint after checking current schema, such as `edit-images-v2` when applicable. Prompt with character identity, direction, target body region, addition placement, rotation/facing, occlusion, and preservation rules. | Do not use `create_map_object` or standalone object generation for fitted paperdoll layers unless the user explicitly asks for an unattached prop. Aseprite extension output modes are editor-specific and are not REST/MCP guarantees. Public REST image edits may return composites; only call a result a layer after verifying a changes-only layer or exported layer file. |
...
- "Paperdoll": gather base image/frame, desired layers, fitted visual additions, target body regions, directions, animations, and isolated-vs-composited output; see `references/paperdolling.md`.
