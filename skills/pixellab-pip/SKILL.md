...
| Top-down terrain/autotile tileset (default; square, isometric, hex, or oblique) | Read `references/tileset.md`, then MCP `create_tiles_pro` with `tile_feature="tileset"`; poll `get_tiles_pro`. | `create-tiles-pro` with `tile_feature: "tileset"`, then `tiles-pro/{tile_id}`. |
| Top-down tileset requiring transition width, per-terrain references, or style matching | Read `references/tileset.md`, then MCP `create_topdown_tileset`; use this route only when those dedicated controls are required. | `create-tileset`, `tilesets`. |
