...
| Skill icon, ability icon, spell icon, action-bar icon, or hotbar icon | REST v2 first. Do not route to object generation, tiles-pro, Pixen, or UI routes by default. | Use `generate-image-v2`, especially for complete/backgrounded icon sheets. For plural/complete skill-icon sets, treat `32x32 icons` as the per-icon cell size, not the output canvas: generate one sheet sized by grid columns times rows, such as `256x256` for an 8x8 sheet of 64 32px icons, then crop/split locally only if separate files are needed. Do not rewrite complete-sheet requests as "each generated image is one standalone icon" unless the user explicitly asks for separate files; that phrasing can flatten backgrounds and push isolated symbol/object art. Do not describe this route as Pixen or Pixen-style. Single backgrounded skill icons are a reasonable default but less directly tested. Prompt for pictorial symbols only, rich full-bleed illustrated miniature backgrounds, fully opaque painted backgrounds when backgrounded icons are requested, no text-like marks, no positive rune/glyph/sigil concepts, no UI slots, no borders/frames/rounded corners unless the user explicitly wants slot/container art. Use UI routes only when the user asks for slots, buttons, bars, panels, or containers. |
