...
| Connectable terrain transition (hex, isometric, oblique, or square top-down) | Read `references/tileset.md`, then MCP `create_tiles_pro` with `tile_feature="tileset"`; poll `get_tiles_pro`. | `create-tiles-pro` with `tile_feature: "tileset"`, then `tiles-pro/{tile_id}`. |
| Dedicated top-down terrain/Wang/autotile tileset | Read `references/tileset.md`, then MCP `create_topdown_tileset`; prefer it when square top-down needs transition width, per-terrain references, or style matching. | `create-tileset`, `tilesets`. |
