...
| Background, scene, backdrop | REST v2 image generation; no hosted MCP background tool is documented. Blind single-reviewer benchmark: **subject-less backdrop → `create-image-pixflux`; full scene with a subject → `create-image-pixen`** (Pixen wins scenes-with-subject, PixFlux wins empty backdrops); Pro is not worth its ~12× cost for scenes/backdrops — see [image-model benchmark results](../../docs/pixellab-image-model-benchmark-results.md). | `create-image-pixflux-background`; verify current size/field support before exact code. |
