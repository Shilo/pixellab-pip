...
- After a successful generation, write a `<name>.blueprint.json` (the route plus the exact request body, replayable and shareable) beside the outputs per `references/blueprint.md`. When the generation used any user-supplied input image, copy that file into the folder by copying the file, not by reading and re-writing it.
