...
Prompts describe visual content or, for action fields, depicted motion — never tool operation, output metadata, or report status. Include only details that change output; omit boilerplate already expressed by a supported control. Prefer positive structural wording and supported controls. Negative prompting includes inline exclusions such as `no text` or `without a border`; a separate negative field is not required. Use a concise inline exclusion when a specific visual boundary matters, but do not add generic negative boilerplate automatically. On Pixen, do not name an otherwise absent object solely to prohibit it; describe the intended empty or replacement state instead. Send `negative_description` only when the live schema exposes it; keep it concise and never invent the field. For a named visual style, state it briefly and avoid conflicting render adjectives; use route-specific references for additional style guidance.
