...
| General image, sprite, standalone asset that is not an icon or emoji | MCP `create_image_pixflux`/`create_image_pixen`/`create_image_pro` + `get_image` when MCP-first — same model choice as REST, minus multi-image style reference (REST-only). For explicit Create Image Pro, `create_image_pro`/`generate-image-v2`, exact grids/sheets, or below-32px cells, read `references/create-image-pro.md` first. Model character: PixFlux (`create-image-pixflux`/`create_image_pixflux`) = lower detail, loose/painterly (frames whole subjects); Pixen (`create-image-pixen`/`create_image_pixen`) = high detail, tight framing; Pixen and Pro crop larger subjects; Pro for style/variety. | `create-image-pixen`, `generate-image-v2`, `create-image-pixflux`, `generate-with-style-v2`. |
