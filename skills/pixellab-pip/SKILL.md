...
- After a successful generation, write a `<name>.blueprint.json` beside the outputs per `references/blueprint.md` — the route, the exact request body, and a `_comment_prompt` recording the user's originating request as a `/pixellab …` command. When the generation used one or more user-supplied input images (any role — source, reference, style, mask, init, frame, and the like), copy each into the folder by copying the file, not by reading and re-writing it.
