...
- After a successful generation, write a `<name>.blueprint.json` (the route plus the exact request body, replayable and shareable) beside the outputs per `references/blueprint.md`. When the generation used one or more user-supplied input images (any role — source, reference, style, mask, init, frame, and the like), copy each into the folder by copying the file, not by reading and re-writing it.
