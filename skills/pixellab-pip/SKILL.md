...
11. `seed`: omit by default; PixelLab randomizes it. Send it in two cases only — the user gave a seed (send verbatim), or several calls share near-identical wording and must land on the same composition (seed-lock): draw one integer with `python -c "import random;print(random.randint(1,2147483647))"` and send that same value on every call in the set. Never ask the user for a seed, and never hand-pick the number.
