...
1. Classify intent; values combine, such as `animate + cost_sensitive`:
   ...
   A standalone `setup`, `update`, `uninstall`, `bark`, or `auto` word after an explicit skill invocation, such as `/pixellab-pip setup` or `@pixellab-pip bark off`, is that intent: for setup read `references/setup.md` via file view, for update read `references/update.md` via file view, for uninstall read `references/uninstall.md` via file view, for bark read `references/bark.md` via file view, for auto read `references/auto.md` via file view.
...
3. Choose the surface with Surface Rules, then the route with the Intent Router. When the user explicitly asks for Aseprite handling, read `references/aseprite-cli.md` via file view; PixelLab MCP/REST generates, documented Aseprite CLI/Lua handles local workspace, import/export, packaging, and launch only.
...
7. Prepare natural-language parameters per Text Preparation. For non-English or mixed-language requests, read `references/localization.md` via file view.
8. For animation, preserve the user's requested frame count; otherwise use the endpoint or template default. Exception: preset/template character animations take no `frame_count`; pick a matching template id (read `references/preset-skeleton-template-animation.md` via file view for catalog) or fall back to v3 custom mode. Preserve PixelLab's returned frame order; no ping-pong, reversed, duplicated, or trimmed outputs unless the user asks for that playback style.
9. If the user says cheap, budget, low-cost, fewer credits, or similar, read `references/cost-routing.md` via file view before choosing a paid route, and ask before each extra paid attempt unless a concrete budget or attempt count was approved.
...
12. Act or answer, then send one final report. Ask a short clarification only for known collisions. Before that report, send only a blocker or a question you need answered — never progress, status, findings, or intentions; those belong in the final report. Do not end a turn waiting on a pending job through an out-of-turn wait (backgrounded poll loop, log/file watcher, scheduled wake) unless that wait is bounded to return control on success, failure, and a hard deadline. Otherwise stop and report the job or asset ID with the status route or getter that resumes the check (read `references/job-lifecycle.md` via file view). Exception — chunk reveal: when an approved run produces several separately-completing image jobs (a multi-shot cinematic chain, an all-directions animation, an approved multi-asset batch), post each job's saved output — path and inline preview link — as that job completes, and fold the last job into the final report rather than revealing it twice. This covers distinct sequential jobs only, not the multiple images a single job returns at once (8-direction character, animation frames, rotations, tileset tiles, review candidates), which go straight to the final report.
...
- Reviewable static candidates: when a static image-style MCP tool or REST endpoint returns multiple alternatives, read `references/reviewable-candidates.md` via file view before selecting, saving, or continuing from one.
...
- Do not post-process PixelLab output into a claimed final asset without explicit approval. Local crop/split/format work that preserves original pixels is allowed when reported honestly; resizing, reassembling, compositing, or repairing failed outputs locally must not be called final without approval. Exception: when a request used `no_background: true` but the output kept a background, read `references/background-removal.md` via file view and attempt safe removal when verification shows the background is removable without changing the art.
- Save downloaded generations, derived previews, manifests, blueprints, and packages under a dedicated asset subfolder inside `pixellab-pip-generations/` (such as `pixellab-pip-generations/<name>/`) unless the user explicitly names another folder. Never write or save generated output files, PNGs, manifests, or `*.blueprint.json` files directly in the repository root directory.
- After a generation returns image(s) or a blueprint is created, read `references/blueprint.md` via file view and write a `<name>.blueprint.json` beside the outputs inside the asset's `pixellab-pip-generations/<name>/` folder following its schema — canonical portable `_pixellab` connection metadata, the exact route bodies, structured `TASK` steps for material work performed outside PixelLab calls, and a `_comment_prompt` holding the user's original prompt as they intended it. Never save a blueprint file directly in the repository root directory.
- After every live generation flow, read `references/usage-reporting.md` via file view and write a manifest beside the outputs inside `pixellab-pip-generations/<name>/` using its layout; keep its private audit/resume data out of the shareable blueprint.
...
For any atlas or spritesheet request with known or requested cell dimensions, also read `references/local-asset-assembly.md` via file view for the required grid inspection preview.
...
| Portrait-to-character or character-to-portrait | MCP `create_portrait_character` + `get_portrait_character` when visible. | `portrait-character-pro` (Pro image conversion). Supplied-image roles: read `references/image-input-roles.md` via file view. |
| Pixel/bitmap font, font atlas | MCP `create_font` + `get_font` when visible. | `generate-font-pro` (Pro). |
| Skill/ability/spell/action-bar/hotbar icon, inventory item/equipment/loot/pickup icon, emoji, or icon sheet | Read `references/icon.md` via file view before choosing an endpoint or generating. | The reference covers route choice, background defaults, sheet sizing, prompt wording, and verification. |
...
| Top-down terrain/Wang/autotile tileset | Read `references/tileset.md` via file view, then MCP `create_topdown_tileset`. | `create-tileset`, `tilesets`. |
| Sidescroller/platformer tileset | Read `references/tileset.md` via file view, then MCP `create_sidescroller_tileset`. | `create-tileset-sidescroller`. |
...
| Fitted paperdoll addition on an existing character image | Treat as an `existing_image` edit anchored on the base frame; read `references/paperdolling.md` via file view before choosing layer/composite outputs. | Do not use object generation for fitted layers unless the user explicitly wants an unattached prop. |
| Style-reference or consistent-style generation | Read `references/style-reference.md` via file view, then REST v2. | `generate-with-style-v2` or `generate-image-v2` style/reference fields after checking current docs. |
| Editor-only utilities (Canny/Pose/Depth, reduce colors, unzoom, pixel correction, reshape) | Read `references/editor-only-utilities.md` via file view. For file-level palette quantization/reduction/replacement, read `references/aseprite-cli.md` via file view even without explicit Aseprite wording. | No public REST/MCP route exists for these; do not invent `/v2/...` routes. |
...
| Preset/template/built-in animation, named motion, or custom skeleton/keypoints | Read `references/preset-skeleton-template-animation.md` via file view; it splits MCP managed-template vs REST raw-skeleton routes. | Do not call website root `/generate-animation/background` or Aseprite extension internals. |
| Auto-rig, estimate skeleton, animate from keypoints | Read `references/preset-skeleton-template-animation.md` via file view. | `estimate-skeleton`, then `animate-with-skeleton`. |
| Raw non-skeleton animation, interpolation, outfit transfer, rotate | REST v2 unless animating a managed MCP character/object. Read `references/animation.md` via file view for frame anchors, idle-loop risk, and verification. | `animate-with-text-v3`, `edit-animation-v2`, `interpolation-v2`, `transfer-outfit-v2`, `rotate`, `generate-8-rotations-v2/v3`. No public 4-rotation route. For a start→end tween prefer `animate-with-text-v3` (`first_frame`+`last_frame`); use `interpolation-v2` only on an explicit Pro/v2 request. |
| Multi-shot, multi-second, or seamless-loop cinematic (a scene longer than one clip) | Read `references/cinematic.md` via file view; requires a user-specified budget, a documented plan, and per-shot validation. | `animate-with-text-v3` — one looped clip for cyclic motion, chained shots (each from the previous handoff frame) for evolving scenes, or `first_frame`+`last_frame` for a strict start→end tween. |
...
| PixelLab projects, sandbox, chat, deployed agents, MCP help/feedback | Read `references/mcp-platform-tools.md` via file view before using `list_projects`, `sandbox_*`, `chat_*`, or `agent_*` tools. | No public REST v2 equivalent is documented. |
| Discover, inspect, select, or replay blueprints/recipes, including a supplied `*.blueprint.json` | Read `references/blueprint.md` via file view and follow its discovery, selection, and replay contract. A blueprint name that contains an asset word (e.g. "knight") is still blueprint intent when the conversation identifies it as one. | The exact route recorded in the blueprint (`MCP <tool>` or `POST /v2/...`). |
...
Read the relevant reference via file view before taking action:
...
Respect documented character limits: many REST v2 description fields allow 2000 characters, but several action/edit/style fields cap at 500. On a length rejection, trim without changing intent, note the adjustment, and retry. Exact limits: read `references/prompt-limits.md` via file view or check OpenAPI.
