...
| Clean up pixel art, quantize/reduce colors, unzoom upscaled art | MCP `correct_pixelart`, `reduce_colors`, `unzoom_image` (0.1 generations each) + `get_image`, else REST v2. Batch an animation's frames or a character's directions into one call so they stay consistent. | `correct-pixelart`, `reduce-colors`, `unzoom`. For file-level palette clamps on local copies, read `references/aseprite-cli.md`. |
