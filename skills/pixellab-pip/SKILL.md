...
| Image edit, inpaint, mask, convert, resize, remove background | REST v2. | `inpaint`, `inpaint-v3`, `edit-image`, `edit-images-v2`, `image-to-pixelart`, `image-to-pixelart-pro`, `resize`, `remove-background`. For exact image-to-pixel-art dimensions, normal `image-to-pixelart` has `output_size`; Pro currently has no output-size field. If Pro quality is required, pre-normalize the input to the requested target size/aspect before sending it to Pro, then verify the result. If it still differs, use `resize` when within the resize endpoint limit, or local nearest-neighbor/canvas postprocessing after download. |
