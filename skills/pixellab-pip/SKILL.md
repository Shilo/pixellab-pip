...
| Hard-projection top-down/south-facing building sprite | Read `references/style-reference.md`; default to REST `POST /v2/generate-with-style-v2` with one neutral screen-aligned reference; use MCP `create_image_pro` with the same guide only as its fallback; apply the reference's hard-gate verification. | Do not route a single sprite to `create_building_kit`; do not promote prompt-only output after repeated projection misses. |
