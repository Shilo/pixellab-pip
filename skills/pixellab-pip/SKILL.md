...
|---|---|---|
| Hosted MCP | Managed PixelLab assets with IDs, polling, downloads, list/get/delete helpers, talking-portrait/lip-sync tools, and map/project/sandbox/agent helpers, including `create_ui_asset`, `create_font`, or `create_portrait_character` when visible; also raw-image primitives `create_image_pixflux`/`create_image_pixen`/`create_image_pro`/`get_image`, `edit_image`, `edit_image_pixen`, `inpaint_image`, `animate_image`, `image_to_pixelart`, and the cleanup tools `unzoom_image`/`correct_pixelart`/`reduce_colors` when visible — these need no managed asset. | REST-only controls such as multi-image style reference (`generate-with-style-v2`), freeform UI (`generate-ui-v2`), stateless lip sync, Pro image-to-pixel-art, or packed spritesheet/ZIP export; resize and remove-background (no MCP tool); or any MCP call when PixelLab MCP tools are not visible. |
...
| UI, HUD, button, panel, health bar, menu | MCP `create_ui_asset` + `get_ui_asset` when MCP-first — it has both `pieces` (rounded_rect/circle/polygon) and `elements` (button, icon_button, toolbar, tab, panel, window, health_bar, avatar, triangle/pentagon/hexagon/octagon); mind its aspect-gated size caps (square ≤512×512, 16:9 ≤688×384, 9:16 ≤384×688, 4:3 ≤600×448, 3:4 ≤448×600). It also has `style_image_base64` and `project_id`, so those no longer force REST; use REST v2 `create-ui-asset` (Pro) when MCP is unavailable or exact schema control is needed. `generate-ui-v2` (REST-only, no MCP tool) for loose/raw UI images, especially with a `concept_image`. | Do not route shape-piece/layout requests to `generate-ui-v2`. |
...
| Clean up pixel art, quantize/reduce colors, unzoom upscaled art | MCP `correct_pixelart`, `reduce_colors`, `unzoom_image` (0.1 generations each) + `get_image`, else REST v2. Pass an animation's frames or a character's directions in one call so they come back consistent with each other; all frames in a call must be the same size, under a per-call pixel budget. Unzoom a user-supplied upscaled image before sending it to any reference/style/init field, and account for its opaque output — see `references/image-input-roles.md`. | `correct-pixelart` (max area 1024×1024), `reduce-colors` (512×512 total pixels across all frames in the call), `unzoom` (bounds in `references/image-input-roles.md`). None of the three declares a size field, so read the endpoint description, not the request schema, for those and for strength, palette, dithering and quantize. For file-level palette clamps on local copies, read `references/aseprite-cli.md`. |
...
| Map image / visual level concept | MCP `create_image_pixflux`/`create_image_pixen` + `get_image` when MCP-first (same subject-vs-subject-less split as the Background row), else REST v2 image/background route; website or Aseprite for map extension workflows. A single flat image is not the tile-based map surface in the row below. | No public map extension/texture surface is documented. |
| Map object | MCP `create_map_object` + `get_map_object`; the object is permanent and can be placed on a map with `place_map_object`. | `POST /map-objects`, then `GET /map-objects/{object_id}` for status + metadata. |
| Whole map, map CRUD, terrain painting, placing objects on a map | MCP only: `create_map` (seeded from a top-down tileset id, which also attaches every connected tileset sharing its tile size and base tile; pass `project_id` when the map is for a game project, or it never syncs into a project workspace and the game cannot load it), `edit_map` (`path`/`rect` ops in terrain cells, terrain `"none"` erases; `dry_run: true` previews the result and warnings without saving), `get_map` (ASCII grid), `view_map` (rendered image), `list_maps`, `place_map_object`/`move_map_object`/`remove_map_object`/`list_map_objects`, `delete_map`. Two terrains from the same family with no tileset blending them render as black tiles — `view_map` names the pair and the call that fixes it. Otherwise the website Map Workshop manually. | No public REST v2 map surface exists; do not invent `/v2/maps...` routes. |
...
- `Gemini`: retired label. It no longer appears in current REST v2 or MCP docs — edit/inpaint cost is now stated as billed by frame grid or image size, and the older website Create Tileset Pro usage is stale. Do not present it as a current tier or provider.
...
| "Make a 512x256 UI panel with a portrait circle and three buttons." | MCP `create_ui_asset` with `pieces`/`elements` (plus `style_image_base64` to match an existing game); REST v2 `create-ui-asset` when MCP is unavailable. |
| "Convert this image to pixel art and remove the background." | MCP `image_to_pixelart` when MCP-first, else REST v2 `image-to-pixelart`; `image-to-pixelart-pro` when no fixed output size is wanted. Then REST v2 `remove-background` (no MCP tool). Unzoom the source first if it is upscaled. |
...
| "Clean up these 8 animation frames and cut them to 16 colors." | MCP `correct_pixelart` then `reduce_colors`, all frames in one call each so they share edges and one palette. |
