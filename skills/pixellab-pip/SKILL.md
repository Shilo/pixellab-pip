...
| General image, sprite, standalone asset that is not a skill/item icon | REST v2. For explicit Create Image Pro, `generate-image-v2`, exact grids/sheets, or below-32px cells, read `references/create-image-pro.md` first. Directional model strengths (guidance, not a hard rule): standalone objects/props → `create-image-pixflux` (Pixen is weakest for props); style/variety → Pro; `create-image-bitforge` trails on raw quality — pick it for its `init_image`/`mask_image`/`forced_palette`/`style_image` controls, not as a default. | `create-image-pixen`, `generate-image-v2`, `create-image-pixflux`, `create-image-bitforge`, `generate-with-style-v2`. |
