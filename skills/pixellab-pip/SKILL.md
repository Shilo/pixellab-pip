...
| REST v2 | Scripts, batch jobs, server integrations, exact endpoint control, BitForge (`coverage_percentage`), multi-image style reference (`generate-with-style-v2`), freeform UI (`generate-ui-v2`), base-tier `edit-image`/`inpaint` weak-guidance controls, prompt enhancement, skeleton-guided animation, animation-frame editing, outfit transfer, single-image rotate, resize, remove background — and any of the MCP-covered work below when MCP tools are not visible. | Guessing SDK methods without checking the installed SDK or current docs. |
...
| General image, sprite, standalone asset that is not an icon or emoji | MCP `create_image_pixflux`/`create_image_pixen`/`create_image_pro` + `get_image` when MCP-first — same model choice as REST, minus BitForge and multi-image style reference (both REST-only). For explicit Create Image Pro, `create_image_pro`/`generate-image-v2`, exact grids/sheets, or below-32px cells, read `references/create-image-pro.md` first. Model character: PixFlux (`create-image-pixflux`/`create_image_pixflux`) = lower detail, loose/painterly (frames whole subjects); Pixen (`create-image-pixen`/`create_image_pixen`) = high detail, tight framing; Pixen and Pro crop larger subjects; Pro for style/variety; `create-image-bitforge` only for `coverage_percentage`, its one unique control (`color_image`/`init_image` are on PixFlux), never a default, and has no MCP equivalent. | `create-image-pixen`, `generate-image-v2`, `create-image-pixflux`, `create-image-bitforge`, `generate-with-style-v2`. |
...
| Image edit, inpaint, mask, convert, resize, remove background | MCP `edit_image` (pro; text instruction or `reference_image_base64`) for an arbitrary image edit, or `inpaint_image` (pro; rectangular `mask_x/y/width/height` or `mask_image_base64`) for a masked-region regenerate, when MCP-first — both are Pro-tier tools (`edit_image` matches REST `edit-images-v2`, `inpaint_image` matches `inpaint-v3`), so route to REST for the cheaper base `edit-image`/`inpaint` tiers or their extra weak-guidance controls. Convert/resize/remove-background have no MCP tool; use REST v2. | `inpaint`, `inpaint-v3` (Pro), `edit-image`, `edit-images-v2`, `image-to-pixelart`, `image-to-pixelart-pro`, `resize`, `remove-background`. |
