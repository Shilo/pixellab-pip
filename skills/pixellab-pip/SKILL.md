...
|---|---|---|
| Hosted MCP | Managed PixelLab assets with IDs, polling, downloads, list/get/delete helpers, and project/sandbox/agent helpers, including `create_ui_asset`, `create_font`, or `create_portrait_character` when visible; also raw-image primitives `create_image_pixflux`/`create_image_pixen`/`create_image_pro`/`get_image`, `edit_image`, `inpaint_image`, `animate_image` when visible — these need no managed asset. | REST-only controls such as UI-asset `style_image`/`project_id`, or multi-image style reference (`generate-with-style-v2`); convert-to-pixel-art, resize, remove-background (no MCP tool); or any MCP call when PixelLab MCP tools are not visible. |
