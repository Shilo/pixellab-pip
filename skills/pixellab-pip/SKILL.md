...
4. Use MCP only if PixelLab MCP tools are available, either bare or prefixed. If MCP is unavailable, route to the matching REST v2 endpoint when one is documented. If MCP and REST v2 are both unavailable or fail, explain why before using any non-PixelLab fallback.
   ...
   PixelLab asset rule: every pixel of the requested art must originate from PixelLab or the user. Local tools may read, download, assemble, package, import/export, preview, verify, mask, pad, crop, resize, format-convert, compute difference masks, and extract changed pixels from PixelLab/user images into transparent layer files. Do not draw, repaint, or synthesize requested content locally unless the user explicitly requests or approves a labeled non-PixelLab fallback.
...
| Fitted paperdoll addition on an existing character image | Use an existing-image edit surface anchored on the base character frame. When the user asks for layers without naming an editor or file format, ask them to choose between separate transparent image layer files plus a final composited image, or an Aseprite/editor layer workflow where each layer contains only the newly added pixels. If the user declines to choose, pick the best available character-anchored route: use verified Aseprite/editor changes-only layers only when the user is already working in that editor or accepts visible editor guidance; otherwise use separate transparent image layer files plus a final composite. For image-file layers, generate each addition as a same-canvas edited composite from the base, then extract only the changed pixels into a transparent layer image and compose the verified layer images over the base. When the user is actively working in Aseprite and the installed extension exposes a changes-only new-layer `output_method`, request that editor output mode and verify the exported layer. For public API/code routes, use a documented REST v2 image-edit endpoint after checking current schema, such as `edit-images-v2` when applicable. Prompt with character identity, direction, target body region, addition placement, rotation/facing, occlusion, and preservation rules. | Do not use `create_map_object` or standalone object generation for fitted paperdoll layers unless the user explicitly asks for an unattached prop. Aseprite extension output modes are editor-specific and are not REST/MCP guarantees. Public REST image edits may return composites; only call a file a layer after verifying it is a transparent changes-only layer image or exported editor layer. Reject extracted layers that include duplicated bodies, broad redraws, or unregistered loose parts. |
...
- Do not use local code or editor automation to create or alter requested visual content for PixelLab asset requests. Examples: PIL/Pillow drawing, canvas/SVG drawing, ImageMagick draw commands, Aseprite Lua drawing, ASCII-to-image, and procedural pixel placement. Local code may still copy, mask, extract, composite, and verify pixels that already came from PixelLab or the user.
