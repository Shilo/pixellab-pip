...
- `setx`, `export`, PowerShell `$env:`, and `ENV=value command` are not inherently forbidden — the risk is the literal Secret appearing in command text. Offer the user both ways to set the `PIXELLAB_SECRET` user variable and let them pick:
  ...
  - **One-line command.** A placeholder-based command the user runs in a normal external terminal, e.g. `setx PIXELLAB_SECRET "<paste-secret>"` (macOS/Linux: add `export PIXELLAB_SECRET="<paste-secret>"` to your shell profile — e.g. `~/.zshrc` on macOS, `~/.bashrc` on Linux — then open a new terminal). Warn that the Secret is then stored in shell/command history; never run it yourself and never fill in a real token.
