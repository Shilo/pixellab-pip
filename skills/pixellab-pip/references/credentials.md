...
- `setx`, `export`, PowerShell `$env:`, and `ENV=value command` are not inherently forbidden — the risk is the literal Secret appearing in command text. Offer the user both ways to set the `PIXELLAB_SECRET` user variable and let them pick:
  ...
  - **One-line command.** On Windows, `setx PIXELLAB_SECRET "<paste-secret>"` in a normal external terminal (persists to the user environment). On macOS/Linux, the persistent form is appending the export line to your shell profile — `echo 'export PIXELLAB_SECRET="<paste-secret>"' >> ~/.zshrc` (macOS) or `~/.bashrc` (Linux) — then open a new terminal; a bare `export` typed into the current shell is session-only and reads as "not set" after restart (and a profile export reaches shell-launched apps, not Finder/Dock-launched ones — for a Dock-launched app, use its own secret settings or launch it from that terminal). The Secret is then stored in plaintext — command history for `setx`, the profile file for the export line — so never run it yourself and never fill in a real token.
