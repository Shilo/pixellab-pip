...
- `setx`, `export`, PowerShell `$env:`, and `ENV=value command` are not inherently forbidden — the risk is the literal Secret appearing in command text. Show placeholder-based commands for a normal external terminal, explain the shell-history/process-history tradeoff, and list secret UIs, secret stores, or hidden prompts first as the safest default. A newly set environment variable reaches only sessions launched afterward, so tell the user to restart the app/assistant from a fresh terminal before any readiness check — otherwise it verifies from a stale session and reports a false "not set".
