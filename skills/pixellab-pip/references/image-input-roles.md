...
|---|---|---|---|
| "Use this as the subject" | Subject reference | The output should depict the same object, character, or subject, while text still guides details. | `reference_images` in MCP `create_image_pro` / REST `generate-image-v2` (same labelled array — MCP takes it as a JSON string, each entry with a `usage` note); `reference_image` in some character routes. |
