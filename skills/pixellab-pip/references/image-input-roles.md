...
| "Describe this pixel art as a PixelLab prompt" | Prompt source image | Analyze the image to produce a PixelLab generation prompt; no image is generated. | REST `POST /image-to-text.image` accepts base64 PNG/JPEG/data URI; there is no MCP equivalent. |
