...
- Concurrency: an account runs a limited number of jobs in parallel. PixelLab grants **priority slots** (queue-skipping concurrent jobs) equal to the account's average concurrency over the trailing 30 minutes, so steady parallel usage raises the ceiling and idle time lowers it. Pace batch work at a steady rate rather than fanning out many paid jobs at once, which trips `429`/`529`. Limits vary by tier and are not fully published — do not assume a specific number.
