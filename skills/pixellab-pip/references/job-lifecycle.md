...
Poll gently to completion: a short initial delay, then back off — not tight loops. A paid async result is already bought, so do not stop at a "reasonable wait" and hand it back; keep polling with your own tools (status route or getter) until the job completes or fails. If your turn cannot last that long, do not stop — continue with a bounded background wait (SKILL.md step 12) that resumes you on completion. Only when the harness can neither hold the turn nor resume you, hand off the job or asset ID and its resuming getter as a last resort, and then say plainly that credits were spent. Stop before "done" only for a real blocker (failure, auth/credit error, or a needed user choice such as review selection) — never just because the job is still running.
