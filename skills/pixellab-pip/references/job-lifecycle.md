...
- Concurrency: an account runs a limited number of jobs in parallel. PixelLab grants queue-skipping **priority slots** for sustained parallel usage over a rolling 30-minute window (≥80% utilization for 30 min earns +1; below 60% for 30 min loses one), so pace batch work steadily rather than fanning out many paid jobs at once, which trips `429`/`529`. Limits vary by tier and are not fully published; no public endpoint reports your slot count, utilization, or active-job list — only per-job `GET /background-jobs/{job_id}` and `GET /balance` (generations/credits). Do not assume a specific number or invent a slots route.
