...
| Reduce colors / quantize palette | Visible PixelLab website/editor, Aseprite, or Pixelorama workflow when the user wants exact PixelLab editor behavior. For explicit local Aseprite palette quantization, source-derived N-color reduction, indexed conversion, strict palette clamps, or document palette replacement, read `aseprite-cli.md`. | Do not invent `/v2/quantize-image`. Distinguish PixelLab editor reduce-colors behavior from documented Aseprite CLI/Lua file handling. |
