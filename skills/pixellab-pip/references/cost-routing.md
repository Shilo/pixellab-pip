...
- Cheap edit and convert: `edit_image_pixen`/`edit-image-pixen` and `image_to_pixelart`/`image-to-pixelart` cost 1 generation. The cleanup family — `unzoom_image`, `correct_pixelart`, `reduce_colors` and their REST twins — costs 0.1 generations per call, and each call takes several frames, so batch rather than loop.
