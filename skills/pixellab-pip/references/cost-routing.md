...
- Skeleton v3 is a separate paid Tier 1+ beta route. MCP docs list 2 generations for 3 raw frames, 3 for 8, and 4 for 15; managed-character `skeleton-v3` documents 2–4 per direction. Check returned usage for the exact charge. Current public USD estimates for the raw endpoint are about `$0.0436`/3 frames, `$0.0513`/8, and `$0.0622`/15.
...
- REST `POST /image-to-text` returns a prompt description, not an image; the public MCP docs estimate about 0.09 generations for a typical sprite or screenshot. The actual usage is returned in the response and varies with answer length.
