...
- When confirming or reporting a live call, show the exact values sent to PixelLab. When a human-readable value (`description`, `action`, and the like) is not already in the user's language, add its translation on the next line so they can both verify and understand it; never duplicate a line already in their language.
