...
- When confirming or reporting a live call, show the English parameter values actually sent to PixelLab. If the user is writing in another language, show each human-readable value (`description`, `action`, and the like) on two lines — the exact English sent first, then its translation in their language on the next line — so they can both verify and understand it; for an English user show the English once, with no duplicate.
