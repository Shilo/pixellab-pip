...
- Beyond managed assets, MCP also documents raw-image primitives needing no managed asset: `create_image`+`get_image` (generic text-to-image, no Pixen/PixFlux/BitForge/Pro model choice), `edit_image` (matches REST `edit-image`), `inpaint_image` (matches REST `inpaint`), and `animate_image` (matches REST `animate-with-text-v3`, and partially `interpolation-v2` via its `last_frame_base64`). MCP still has no tool equivalent to REST v2 `edit-images-v2` (multi-source combine), `inpaint-v3` (Pro tier), `image-to-pixelart`, `image-to-pixelart-pro`, `resize`, or `remove-background`. Route those to REST v2 unless a matching MCP tool is visible; do not assume a REST endpoint has an MCP equivalent just because MCP is configured.
