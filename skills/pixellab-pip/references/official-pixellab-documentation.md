...
- Beyond managed assets, MCP also documents raw-image primitives needing no managed asset: `create_image_pixflux`, `create_image_pixen`, `create_image_pro` (each a dedicated model, matching REST `create-image-pixflux`/`create-image-pixen`/`generate-image-v2` on their core fields — a few REST-only extras exist per route: `negative_description`, `background_removal_task`, `enhance_prompt`; `create_image_pixflux` also covers `create-image-pixflux-background`, a byte-identical schema), sharing `get_image` as their getter; `edit_image` — **Pro tier**, matches REST `edit-images-v2` (batch of 1-16 images + text-or-reference mode), not base `edit-image` (single image, text-only, has `color_image`/`text_guidance_scale` that `edit_image` lacks); `inpaint_image` — **Pro tier**, matches REST `inpaint-v3` (has `crop_to_mask`, a field that exists only on v3), not base `inpaint` (which has extra weak-guidance controls `inpaint_image` lacks: `direction`/`isometric`/`shading`/`outline`/`detail`/`text_guidance_scale`/`init_image`/`color_image`); and `animate_image` (matches REST `animate-with-text-v3`, and partially `interpolation-v2` via its `last_frame_base64`). MCP still has no tool equivalent to REST v2 `create-image-bitforge` (`coverage_percentage`), `generate-with-style-v2` (multi-image style reference), `generate-ui-v2` (freeform UI — `create_ui_asset` is structured-only), base-tier `edit-image`/`inpaint` (their extra weak-guidance controls are unreachable via MCP), `image-to-pixelart`, `image-to-pixelart-pro`, `resize`, `remove-background`, or `rotate` (single arbitrary rotation). Legacy `animate-with-text`/`animate-with-text-v2` (`reference_image` is a subject/style role, not a frame anchor) are only partially covered via managed `animate_character`/`animate_object` `mode="v3"`; `generate-8-rotations-v2`/`-v3` only partially via `create_8_direction_object`/`create_character(n_directions=8)`, which regenerate rather than rotate the exact input pixels. Route the fully-REST-only ones to REST v2 unless a matching MCP tool is visible; do not assume a REST endpoint has an MCP equivalent just because MCP is configured.
