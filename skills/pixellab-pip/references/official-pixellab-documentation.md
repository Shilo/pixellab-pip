...
- Beyond managed assets, MCP documents raw-image primitives needing no managed asset: `create_image_pixflux`/`create_image_pixen`/`create_image_pro` (+ `get_image` as their shared getter, matching REST `create-image-pixflux`/`create-image-pixen`/`generate-image-v2` on core fields; `create_image_pixflux` also covers `create-image-pixflux-background`, a byte-identical schema), `edit_image` (**Pro tier** — matches `edit-images-v2`, not base `edit-image`), `inpaint_image` (**Pro tier** — matches `inpaint-v3`, not base `inpaint`; base `inpaint` has extra weak-guidance style controls MCP can't reach), and `animate_image` (matches `animate-with-text-v3`, partially `interpolation-v2` via `last_frame_base64`).
- MCP has no tool for REST `create-image-bitforge`, `generate-with-style-v2`, `generate-ui-v2`, base-tier `edit-image`/`inpaint`, `image-to-pixelart`(`-pro`), `resize`, `remove-background`, or `rotate`. Legacy `animate-with-text`/`-v2` and `generate-8-rotations-v2`/`-v3` are only partially covered via managed `animate_character`/`animate_object`/`create_8_direction_object`, which regenerate rather than exactly reproduce the input. Route the REST-only ones to REST v2; do not assume a REST endpoint has an MCP equivalent just because MCP is configured.
