...
Use MCP `pixelart_workbench`; this is an MCP-only tool, not a REST route. It operates on PixelLab-managed image IDs, not local file paths. Pass one command in `argv` per call (string or argument list), use low mode by default, and use returned IDs for follow-up operations. Optional `notes` are stored with the step; include them on each call where they are needed. Inspect the resulting art before reporting completion. The MCP tool description says PixelLab's Workbench commands are free for subscribers; this does not cover the preferred AI model's token cost.
