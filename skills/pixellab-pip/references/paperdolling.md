...
| Fitted paperdoll edit from code/API | Prefer MCP `edit_image` (pro; `images_base64`+`description`, or `reference_image_base64`) on the base image; fall back to REST `edit-images-v2` (or base-tier `edit-image` for its cheaper tier / `color_image`+`text_guidance_scale` controls) when MCP is not visible or those extras matter. Detailed character-anchored edit prompt, same canvas size. Treat the result as an edited composite first. | Public REST/MCP image-edit routes are not editor layer workflows; they output edited images, not `output_method` layer modes. |
...
| Masked fitted layer attempt | Visible editor image-edit workflow with a body-region selection and changes-only layer output when available, or MCP `inpaint_image` (pro; rectangular `mask_x/y/width/height` or `mask_image_base64`) / REST `inpaint-v3` only when the user supplies or approves a mask for the body region. | Inpainting returns an edited image, not semantic layer extraction. Verified editor changes-only output is the better layer path. |
...
| Existing composite to isolated layer | Require an explicit mask or editor cleanup plan; use MCP `inpaint_image`/`edit_image` or REST `inpaint`/`edit-image` only as a fallback. | PixelLab does not document semantic layer extraction, character subtraction, or occluded-addition reconstruction. |
...
- MCP `inpaint_image` is the MCP-first equivalent of `inpaint-v3` (both Pro; both carry `crop_to_mask`), taking `image_base64`+`description`+rectangular mask or `mask_image_base64`. REST `inpaint-v3` is documented as Pro (cost signal per SKILL.md Model And Mode Terms), requires `description`, `inpainting_image`, and `mask_image`; white mask areas are generated/replaced, black areas preserved. Output is a whole edited image, not a layer. `context_image` and `bounding_box` are deprecated in current OpenAPI.
- MCP `edit_image` is the MCP-first equivalent of `edit-images-v2` (pro): `images_base64` (1-16, same edit applied consistently) with `description` for text mode or `reference_image_base64` for reference mode. REST `edit-images-v2`'s `method` enum is exactly `edit_with_text`/`edit_with_reference` for the same 1-16 images (depending on size); treat output as an edited image/composite unless a transparent layer is extracted and verified, or a changes-only editor layer is verified.
...
- Route MCP-managed characters through character/state/animation tools; for raw frames prefer MCP `animate_image` (`first_frame_base64`+`action`, optional `last_frame_base64` — also partially covers interpolation via its end-frame anchor); use REST `edit-animation-v2` and `transfer-outfit-v2` (no MCP equivalent) or REST raw animation only when exact file-level control matters. Warn that text-only paperdolling drifts without a base frame, seed, or reference.
