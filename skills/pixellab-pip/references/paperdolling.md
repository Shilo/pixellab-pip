...
PixelLab can generate base sprites, edited states, outfit-transfer frames, masked inpaints, image edits, and standalone transparent objects. Public REST v2/MCP docs do not expose first-class layer creation, layer assignment, semantic layer extraction, or isolated changed-part outputs. Some editor integrations, including the Aseprite extension, may expose an editor-local output mode that requests a new layer containing only changed pixels. Treat that as an editor-specific capability, not a REST/MCP contract, and accept the result only after visual/export verification proves it is actually changes-only.
