...
For 1-bit edge-placement prompt work, avoid generic `white border around the tile` wording. In a connected tileset this can imply borders around every source tile and create repeat seams. For top-down wall/floor or cave/island tests, put white rim, white contour, or white boundary pixels in `transition_description`; keep `lower_description` and `upper_description` focused on the two terrain masses. For sidescroller platform tests, keep `lower_description` focused on the repeatable center/platform body and put white top/rim/end-cap pixels in `transition_description`; explicitly say `never on repeatable middle seams` when seam cleanliness matters.

For top-down 1-bit wall/floor tests, a strong starting point is black lower terrain, black upper terrain, `transition_size: 0.5`, and a transition description such as `one-pixel pure white rim pixels on the boundary where wall meets floor, white only on exposed contour edges, black interior, no gray`. For sidescroller 1-bit ground tests, a strong starting point is a black horizontally tileable `lower_description`, `transition_size: 0.25`, and a transition description such as `pure white broken rim pixels only on exposed top surface and outside end-cap edges, never on middle tile seams, black interior, no gray`.
