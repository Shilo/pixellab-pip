...
For strict 1-bit edge-placement work, do not use `color_image` as the automatic first fix for missing white rims or contours. Live REST checks showed black/white `color_image` can collapse a black terrain plus white transition request into all-black output, erasing the requested transition. Test shape and placement first; add palette controls only as a separate verified follow-up, or report a palette-clamped derivative as local processing after saving the untouched PixelLab output.
