...
- [PixelLab MCP vs REST v2 Route Parity](pixellab/pixellab-mcp-vs-rest-route-parity.md) - endpoint-level MCP/REST coverage matrix and the full inventory of REST v2 endpoints with no MCP counterpart.
...
- [PixelLab 16px Item Sprite Generation Spike](pixellab/pixellab-16px-item-sprite-generation-spike.md) - findings on why `16x16` full-cell tiles work better than strict `16x16` non-tile item sprites, including prompt comparisons, Aseprite style-reference observations, and verification guidance.
