...
| Image edit, inpaint, conversion, resize, remove background | In-editor editing, mask/inpaint, conversion, resize, and background removal; observed operation names include `generate-edit`, `generate-edit-image-pro`, `generate-inpainting`, `generate-inpainting-v3`, `generate-image-to-pixelart`, `generate-image-to-pixelart-pro`, `generate-resize`, and `remove-background`. | MCP `edit_image` (Pro, matches REST `edit-images-v2` — batch of 1-16 images + text-or-reference mode) and `inpaint_image` (Pro, matches REST `inpaint-v3` — has `crop_to_mask`) when MCP-first; REST v2 for base-tier `edit-image`/`inpaint` (their extra weak-guidance controls aren't reachable via MCP), `image-to-pixelart`(`-pro`), `resize`, `remove-background`. |
