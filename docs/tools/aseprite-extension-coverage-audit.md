...
- Hosted MCP tools for managed characters, portrait characters, objects, tilesets, tile variants (including path tiles and building kits), map objects, fonts, structured UI assets, raw image generate/edit/inpaint/animate, chat/sandbox helpers, projects, and balance.
- REST v2 endpoints under `https://api.pixellab.ai/v2` for code/API work.
- Aseprite/Pixelorama/website for editor-only workflows.

The installed extension review now shows a PixMiniMax animation/interpolation workflow with 4-frame increments, a 40-frame maximum, a 256px canvas ceiling, and a returned reference frame alongside generated frames. Those editor observations align with the current public PixMiniMax shape, but the extension's private transport remains excluded from Pip's automation contract; use public REST/MCP instead.
...
- Observed extension workflow families include image generation, style/reference generation, multi-image edit, quantize/reduce-colors, unzoom, pixel correction, map-extension, rotation, animation, tileset, UI, update, and review workflows.
...
- Official Aseprite scripting docs and API docs document Lua scripting, `app.open`, `Sprite()`, `Sprite:saveCopyAs()`, `Sprite:setPalette()`, `Sprite:loadPalette()`, `Sprite.transparentColor`, `Palette()`, `Palette:setColor()`, `Palette:saveAs()`, `app.command.LoadPalette`, `app.command.ColorQuantization`, `app.command.ChangePixelFormat`, dialogs, and plugin/menu extension points: <https://www.aseprite.org/docs/scripting/> and <https://www.aseprite.org/api/>.
- Official REST v2 docs list public paths under `/v2`, such as `/generate-image-v2`, `/edit-images-v2`, `/inpaint-v3`, `/animate-with-text-v3`, `/animate-pixminimax`, `/create-tileset`, `/create-tiles-pro`, `/resize`, and `/remove-background`.
...
|---|---|---|
| General image, Pixen, style reference, Flux-like image generation | In-editor image generation with optional references and style controls; observed operation names include `generate-image-new`, `generate-image-pixen`, `generate-pixelart-flux`, `generate-consistent-style`, `generate-flux-same-style`, and `generate-style`. | MCP `create_image_pixflux`/`create_image_pixen`/`create_image_pro` (+ `get_image`) when MCP-first; REST v2 for BitForge (`create-image-bitforge`, no MCP equivalent) and multi-image style reference (`generate-with-style-v2`, no MCP equivalent); Aseprite for exact in-editor behavior. |
| UI generation | In-editor UI element and layout-assisted generation; observed operation names include `generate-ui` and `generate-ui-template`. | REST v2 `generate-ui-v2` (no MCP equivalent — MCP's `create_ui_asset` is structured-only); website/editor for visual UI-library flows. |
| Image edit, inpaint, conversion, resize, remove background | In-editor editing, mask/inpaint, conversion, resize, and background removal; observed operation names include `generate-edit`, `generate-edit-image-pro`, `generate-inpainting`, `generate-inpainting-v3`, `generate-image-to-pixelart`, `generate-image-to-pixelart-pro`, `generate-resize`, and `remove-background`. | MCP `edit_image_pixen` (1 generation, matches REST `edit-image-pixen`), `edit_image` (Pro, matches REST `edit-images-v2` — batch of 1-16 images + text-or-reference mode), `inpaint_image` (Pro, matches REST `inpaint-v3` — has `crop_to_mask`), and `image_to_pixelart` when MCP-first; REST v2 for base-tier `edit-image`/`inpaint` (their extra weak-guidance controls aren't reachable via MCP), `image-to-pixelart-pro`, `resize`, `remove-background`. |
| Multi-image edit | In-editor edit using multiple supplied images; observed operation name `generate-multi-edit`. | MCP `edit_image` (preferred image URLs or inline base64, with optional reference mode) when MCP-first; REST v2 `edit-images-v2` for exact schema control; website/editor for visual experimental flows. |
| Animation, interpolation, skeleton, outfit transfer | In-editor animation generation/editing, skeleton-guided workflows, interpolation, and outfit transfer; the current extension also exposes a PixMiniMax-labeled animation/interpolation workflow. | MCP `animate_image` for raw v3-style animation and `animate_image_pixminimax` for explicit PixMiniMax/MiniMax H3 animation (both no managed asset needed), plus MCP animation tools for managed characters/objects; REST v2 for `animate-with-text-v3`, `animate-pixminimax`, skeleton-guided animation, `edit-animation-v2`, `interpolation-v2`, and `transfer-outfit-v2`. Private editor operation names remain terminology evidence only. |
...
| Tiles, tilesets, isometric tiles | In-editor tile, tileset, sidescroller, isometric, and tile-variant workflows; observed operation names include `generate-tiles`, `generate-tiles-style`, `generate-tileset`, `generate-tileset-sidescroller`, `generate-isometric-tile`, and `generate-tiles-pro`. | MCP tileset/tile tools by default, including `create_path_tiles` (connectable path/road set) and `create_building_kit` (connectable walls/floor/doorways) — both siblings of `create_tiles_pro` with no REST endpoint or getter of their own; REST v2 `create-tileset`, `create-tileset-sidescroller`, `create-isometric-tile`, `create-tiles-pro`. |
| Map image, map extension, texture | In-editor map image, map-extension, and texture workflows; observed operation names include `generate-map-flux`, `generate-inpainting-map`, `generate-inpainting-map-v2`, and `generate-texture`. | REST v2 image/background or tileset routes for generated assets; MCP map tools (`create_map`, `edit_map`, `view_map`, and the placement tools, first seen in the 2026-09-08 refresh) for a real tile-based map; website/Aseprite for exact map-extension or texture editor workflows. Map extension and texture generation still have no public route, and REST still has no map surface at all. |
...
- Quantize, unzoom, and pixel correction now route to their public REST/MCP endpoints, first seen in the 2026-09-08 refresh; before that they were described as extension-only PixelLab editor workflows rather than local tooling.
