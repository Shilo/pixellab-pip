...
A Pip **blueprint** (`references/blueprint.md`) is the minimal shareable record of a PixelLab workflow: one `*.blueprint.json` holds exact `MCP <tool>` or `POST /v2/...` request bodies plus ordered `TASK` instructions for material work outside those calls. It is both backward-looking (captures the successful replay path) and forward-looking (replay it with plain-language overrides), needs no bundled runner—the host agent executes it—and is portable: one file is the shareable unit, with referenced files alongside. Human-authored blueprints in the skill's `blueprints/` folder can be run by name; `_comment*` keys remain non-executable notes.

So the overlap is the replay idea; the divergence is intent and packaging. Recipes optimize for **producing a parameterized pack through bundled tooling**; blueprints optimize for **reproducing and sharing an ordered PixelLab workflow in any agent, with no bundled runner**. Pip deliberately does not ship templated packs with seed offsets or its own runner; array order, exact request bodies, relative files, and flexible agent tasks cover the portable replay slice.
...
- **Adopt (done): blueprint system.** Added 2026-07-06 and extended with task steps: `*.blueprint.json` records exact PixelLab requests plus material outside-call work as ordered `TASK` instructions, is auto-written after each run, supports plain-language overrides, and remains portable without a bundled runner (`references/blueprint.md`). Templated multi-asset packs with placeholder dependencies, seed offsets, and budgets remain deliberately out of scope. See [Blueprints vs Recipes](#blueprints-vs-recipes).
