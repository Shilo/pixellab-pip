...
| Main asset coverage | Characters, portrait-character conversion, stored portraits, vocal visemes, talking GIFs, lip-sync plans, fonts, character states, animations, tiles and tilesets, UI assets, objects, map objects, tile-based maps and map placement, raw image generate/edit/inpaint/animate, pixel-art cleanup (unzoom, correct, reduce-colors), image-to-pixel-art, balance, projects, chat, sandbox, job control, and agent helpers. | Covers MCP assets plus REST v2, including stateless lip sync, website/editor-only flows, Aseprite, Pixelorama, SDK/API questions, and legacy v1 compatibility. |
