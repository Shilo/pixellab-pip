...
The storm arc took the chained/evolving route and produced a **genuinely striking calm→storm progression**: seven keyframes carried the scene from a golden calm evening through gathering overcast, building storm, rain, a lit sweeping beam, and a final lightning strike, with the palette shifting gold → grey → stormy blue-black and the sea escalating from calm to crashing. The composition held across the keyframes because they were **seed-locked** — each keyframe generated with the same seed and the same composition wording, changing only the palette/weather words. Notably, an init-image *edit of the opening frame* was tried first and **discarded**: it anchored to the source palette and would not cross to the storm colours. The `last_frame` tween between keyframes then carried the actual gold→storm colour cross.

The finished piece is a real ~30-second one-way arc — **289 frames at ~10 fps, 30.06 s, for ~$1.05 total** (18 chained jobs plus a few discarded probes). So an evolving cinematic is **not dollar-expensive**; the real cost is **time and drift**: it ran well over an hour and re-did its whole chain once when the first pass wandered, versus two quick calls for a loop. That is the round's clearest lesson — loops are cheap, fast, and seamless, while an evolving one-way arc is affordable but slow and drift-prone, and holding the fixed camera depends on **seed-locking** the keyframes (not editing the opening frame, which resists a large palette change). Some composition drift still appeared in the free-run storm climax (waves grew dominant and the lighthouse silhouette softened) — an honest limit of long free-run tails, mitigated by keyframing more densely through the most active part.
