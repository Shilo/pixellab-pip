...
The storm arc took the chained/evolving route across seven distinct keyframes (golden calm → dusk → overcast → building storm → storm → beam → lightning). The **anchored middle** of the sequence — where each job is pulled to the next keyframe — walks that gold→storm evolution cleanly and reads as genuinely cinematic. Getting the palette to cross at all required **seed-locking** the keyframes (same seed + composition wording, varying only the palette/weather words); an init-image *edit of the opening frame* was tried first and **discarded** because it anchored to the source palette and would not cross to the storm colours. The `last_frame` tween between keyframes then carried the actual colour cross. The finished piece is a real ~30-second one-way arc — **289 frames at ~10 fps, 30.06 s, for ~$1.05 total** (18 chained jobs plus a few discarded probes) — so an evolving cinematic is **not dollar-expensive**; the real cost is **time and drift**.

**Forensic follow-up — the arc is flawed at both ends.** A close frame-by-frame audit found the good anchored middle is bookended by two failure zones, traced to specific and fixable authoring mistakes:

- **Long free-run stretches drift and accumulate artifacts.** The climax was **9 consecutive free-run jobs** (no `last_frame`): the wave foam compounds frame-over-frame and never clears, blooming into white mushroom-cloud "explosions" and then a static white coating over the islet, with the palette creeping back toward gold during "stormy night." The 3-job free-run opening is ~5 near-frozen seconds. *Fix: anchor on a cadence — never chain more than ~2–3 free-run jobs; re-pass a stable keyframe to pull error back.*
