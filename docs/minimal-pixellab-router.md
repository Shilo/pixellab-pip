...
3. For a queued MCP job, poll its callable `get_*` or status tool until `completed`, `failed`, or an actionable state such as `review`; at `review`, stop for user selection. If no status tool is callable, report that the job cannot be monitored and do not submit a duplicate through REST.
4. Otherwise resolve the matching operation from `https://api.pixellab.ai/v2/openapi.json`, including its method, path, parameters, request and response schemas, and content types. Call it at `https://api.pixellab.ai/v2` using only documented fields. Require `PIXELLAB_SECRET` in the process environment and use it only as `Authorization: Bearer <token>`; never ask for or expose it in command text, output, logs, or files. If it is absent, stop without performing authentication setup.
5. For every background job ID returned by REST, poll authenticated `GET https://api.pixellab.ai/v2/background-jobs/{job_id}` every 5–10 seconds until `completed` or `failed`. On completion, handle `last_response` when non-null; if it is missing or null, report completion without result data. If no callable MCP tool or documented REST v2 operation supports the request, report it as unsupported.
```
