...
Results land under `.local/bench/<stamp>/` (`SUMMARY.md`, `results.json`, `static.json`, per-cell responses); `--rescore <dir>` recomputes checks/summary offline. `.local/` is gitignored, so reports stay on the machine that ran them and are never committed. Workspaces are materialized outside the repo so CLAUDE.md/AGENTS.md cannot contaminate arms; reps interleave variants so provider caching hits both alike; medians are reported. Dry scenarios forbid network use and run with read-only tools.

Only two scenarios call PixelLab, and both are opt-in. `--live` adds the free read-only `GET /balance` scenario (requires `PIXELLAB_SECRET`); the one credit-spending scenario (a single cheap generation) additionally requires `--allow-paid`. Every other scenario is dry and makes no PixelLab call. Note that non-`--static` scenarios still call the agent CLIs themselves (`claude`/`codex`/`opencode`), which consume that provider's tokens — that is the cost being measured. `--static` calls nothing.

On Windows, [dev-tools/run-skill-benchmark.ps1](../dev-tools/run-skill-benchmark.ps1) wraps the common presets so you do not have to remember flags. It preflights for `python`, the required agent CLIs, and `PIXELLAB_SECRET`, confirms before any paid run, and prints the report path when done. Run it with no argument for a menu, or pass a preset:

```powershell
.\dev-tools\run-skill-benchmark.ps1 -Preset static        # free, no CLI, no secret
...
The `full` preset spends real money. At its default `-Reps 1` it runs 3 agents x 4 variants x 15 scenarios = 180 agent calls, of which 12 are credit-spending PixelLab generations (the one paid scenario across every agent/variant) plus 12 free balance checks; `-Reps 3` triples that to 540 calls and 36 paid generations. The interactive launcher asks `skill_benchmark.py --print-plan` for the exact paid-generation count and makes you confirm it before spending. Expect on the order of a million-plus agent input tokens per proprietary-model agent per rep and ~12 PixelLab credits at the default; deepseek-v4-pro via OpenCode is far cheaper per token. Raise `-Reps` for tighter medians, or scope with `--scenarios`, to trade time and credits for stability. (The raw `skill_benchmark.py --reps` default is 3; the launcher passes 1.)

Most scenarios are dry routing checks (no PixelLab call, no credits) — the agent names the route it would take and deterministic regexes score it. Several are derived from the [showcase](showcase/README.md) results and target the cases that separate the skill from the official docs: complete icon/tile **sheets** routing to REST `generate-image-v2` (where a pure-MCP agent tends to pick `create_ui_asset`/`create_tiles_pro`), the tiles-vs-tileset distinction, local 1-bit/palette recolor as Aseprite processing rather than a PixelLab call, and the background-removal fallback. Only `live-cheap-image` spends a credit.
