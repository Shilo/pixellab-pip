...
The `full` preset is exhaustive but **spends no PixelLab credits** (every scenario is dry). At its default `-Reps 1` it runs 3 agents x 4 variants x 12 scenarios = 144 agent calls; `-Reps 3` triples that to 432. Its only cost is agent-provider tokens — on the order of a million-plus input tokens per proprietary-model agent per rep; deepseek-v4-pro via OpenCode is far cheaper. Raise `-Reps` for tighter medians, or scope with `--scenarios`. (The raw `skill_benchmark.py --reps` default is 3; the launcher passes 1.)

All 12 scenarios are dry routing checks — the agent names the route it would take and deterministic regexes score only the **exact** correct tool. Several are derived from the [showcase](showcase/README.md) results and target the cases that separate the skill from the official docs: complete icon/tile **sheets** routing to REST `generate-image-v2` (where a pure-MCP agent tends to pick `create_ui_asset`/`create_tiles_pro`), the tiles-vs-tileset distinction, local 1-bit/palette recolor as Aseprite processing rather than a PixelLab call, and the background-removal fallback.
