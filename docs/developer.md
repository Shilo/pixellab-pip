...
The `full` preset is deliberately exhaustive and spends real money: at `-Reps 3` it runs 3 agents x 4 variants x 15 scenarios x 3 reps = 540 agent calls, of which 36 are credit-spending PixelLab generations (the one paid scenario across every agent/variant/rep) plus 36 free balance checks. The interactive launcher asks `skill_benchmark.py --print-plan` for the exact paid-generation count and makes you confirm it before spending. Expect on the order of a few million agent input tokens per proprietary-model agent and ~36 PixelLab credits; deepseek-v4-pro via OpenCode is far cheaper per token. Lower `-Reps`, or scope with `--scenarios`, to cut spend.
