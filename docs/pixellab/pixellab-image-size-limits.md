...
| `create-image-pro-flash` | `image_size` | 16×16 | 256×256 | Presets: 16×16 experimental, 24×24, 32×32, 32×48, 64×64, 96×64, 96×96. Custom dimensions in multiples of four are Beta. One image per call; check the live capability lookup before selecting a size. |
...
| `create-character-pro-flash` | optional `image_size` | 16×16 | 256×256 | Text creation defaults to 64×64; same Pro Flash presets/custom Beta dimensions as image creation. Eight directions only; input pixels may be transparently padded to the V3 square canvas. Schema-read, not live-tested. |
...
| `create-object-pro-flash` | optional `image_size` | 16×16 | 256×256 | Text creation defaults to 64×64; same Pro Flash presets/custom Beta dimensions. One or eight directions. Schema-read, not live-tested. |
...
The Pro Flash MCP page lists `width`/`height` but not their full validation bounds. Its `get_pro_flash_capabilities` tool is the documented way to check exact native size pairs. The Pro Flash REST rows above come from OpenAPI; no live Pro Flash generation or connected MCP schema probe was performed here.
...
| `16px` object | One direction is schema-supported; eight directions are not on the older object route | `create-1-direction-object` minimum fell to 16 in the 2026-09-08 schema refresh; `create-8-direction-object` minimum is 24. The earlier 32px rejection probes predate that change. Pro Flash also lists experimental 16×16 for one or eight directions, but neither output is live-verified. |
...
Those 2026-07 follow-up limits were resolved; the newer Pro Flash bounds and 16px output quality have not been live-probed here. Do not promote an `8px` icon/item/tileset claim — it is confirmed rejected; the only 8px-native path is font `glyph_px: 8`.
