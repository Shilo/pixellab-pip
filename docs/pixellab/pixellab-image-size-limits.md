...
| `create-1-direction-object` | `size` | 32 | 256 | Probe-confirmed: 8, 16, 31 → `422 ge 32`. Default 64. Mutually exclusive with `style_images` (max 256×256 each); size drives object count. |
| `create-8-direction-object` | `size` | 32 | 168 | Probe-confirmed: 8, 16, 31 → `422 ge 32`. Default 64; the 8-rotation pipeline rejects anything larger than 168. |
...
| `create_image_pixflux` | `width` / `height` | 128 / 128 (or `init_image_base64`'s size) | **none declared** — prose only | Matches REST `create-image-pixflux`'s `image_size`; no MCP schema bound found, unlike REST's declared enum/range |
| `create_image_pixen` | `width` / `height` | 128 / 128 | **none declared** — prose only | Matches REST `create-image-pixen`'s `image_size`; same caveat |
| `create_image_pro` | `width` / `height` | 128 / 128 | **none declared** — prose: "Max depends on aspect ratio (512x512 square, 688x384 for 16:9)" | Prose gives only two aspect buckets; REST `generate-image-v2` documents a wider ladder (e.g. `792×688` extreme-wide) — verify against REST if an aspect outside square/16:9 is needed |
