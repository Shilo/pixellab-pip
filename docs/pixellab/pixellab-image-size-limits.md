...
| `create_image_pixen` | `width` / `height` | 128 / 128 | 16–768 | Range matches REST `create-image-pixen`, but REST also declares `multipleOf: 4` and MCP does not — a non-multiple-of-4 size passes the MCP schema. Both cap total area at 512×512 (prose) |
