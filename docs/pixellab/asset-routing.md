...
| Image edit, inpaint, remove background, resize, convert to pixel art | REST v2 edit/transform endpoints. | Supplied images are optional unless the selected route requires image fields. For fixed-size image-to-pixel-art output, normal `image-to-pixelart` has `output_size`; `image-to-pixelart-pro` does not expose output dimensions. If Pro is needed, pre-normalize the input image to the requested target size/aspect, then verify and normalize the result with `resize` when within its limit, or local nearest-neighbor/canvas postprocessing for larger backgrounds. |
