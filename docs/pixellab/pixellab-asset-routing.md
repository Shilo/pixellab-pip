...
| Skill, ability, spell, action-bar, or hotbar icon | REST v2 `generate-image-v2` first. | Best-supported for complete/backgrounded sheets; reasonable default for single backgrounded skill icons, but less directly tested. For plural/complete requests, `32x32 icons` means per-icon cell size, not a 32x32 output canvas; use a sheet canvas such as `256x256` for 64 icons in an 8x8 grid. See [PixelLab Skill Icon Generation Spike](pixellab-skill-icon-generation-spike.md): current live-generation winner is REST `generate-image-v2`, not object generation, tiles-pro, Pixen, or UI routes. Do not call this Pixen/Pixen-style. Do not rewrite complete sheets as "each generated image is one standalone icon" unless the user explicitly wants separate generated files. Avoid positive rune/glyph/sigil concepts and preserve rich full-bleed background wording. Do not route to UI asset tools unless the user asks for slots, buttons, bars, panels, or containers. |
