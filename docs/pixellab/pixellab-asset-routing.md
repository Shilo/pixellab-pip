...
| General image, standalone sprite, icon, concept that is not a skill icon or item icon | MCP `create_image` + `get_image` when MCP-first (one general-purpose model, no Pixen/PixFlux/BitForge/Pro choice); REST v2 image generation for model choice or Pro quality. | Route skill/ability/spell/action-bar/hotbar icons through the skill-icon reference, and inventory/equipment/loot/pickup icons through the item-icon reference. |
