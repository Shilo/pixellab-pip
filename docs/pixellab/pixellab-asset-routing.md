...
| Animate an existing image/sprite directly (no managed character/object) | MCP `animate_image` (`first_frame_base64`+`action`, optional `last_frame_base64` for a tween) when MCP-first; REST v2 `animate-with-text-v3` otherwise. | Previously raw-image animation had no MCP tool and required REST or first creating a managed asset; `animate_image` now covers it directly. |
| Multi-image edit or combine references | MCP `edit_image` when MCP-first (`images_base64` takes several frames and applies the same edit consistently; `reference_image_base64` switches to reference mode); REST v2 `edit-images-v2` otherwise. | MCP `edit_image` matches the `edit-images-v2` (Pro) field shape, not base `edit-image`. Aseprite's observed `generate-multi-edit` operation is an undocumented internal endpoint, not a public REST route. Use website/editor only when the user wants the visual experimental product flow. |
