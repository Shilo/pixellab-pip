...
|---|---|---|
| Character, player, NPC, enemy, creature | MCP character tools when configured; REST v2 character endpoints for code or exact control. | Character workflows often involve identity, directions, states, animations, and managed asset IDs. Do not treat Create Image S-XL (new)/Pixen as equivalent to managed Character Creator v3: Pixen/v3/new may underweight user instructions, including view, direction, and shape details. For static south-facing or explicit user shape/feature details, prefer Create Image Pro (`create_image_pro`/`generate-image-v2`) when the higher cost and different, broader character style are acceptable; it follows the user's description more closely than Pixen/v3/new. Visually verify it. |
...
| Talking portrait, mouth shapes, talking GIF, or lip-sync plan | MCP talking-portrait tools when configured; REST v2 for code, stateless lip sync, or exact control. | Attach a portrait, generate one mood's visemes with the only paid call, then render a free synchronous GIF or request a free timing plan. REST `/lip-sync` alone supports stateless `viseme_count`. Preserve dialogue verbatim. |
...
| Connectable path/road tile set | MCP `create_path_tiles` or REST v2 `create-tiles-pro` (`tile_feature: "roads"`). | Shares the tiles-pro get/list/delete tools; no dedicated getter. |
| Building kit (floor, connectable walls, doorways, pillar, stairs) | MCP `create_building_kit` or REST v2 `create-tiles-pro` (`tile_feature: "building"`). | Shares the tiles-pro get/list/delete tools; no dedicated getter. |
...
| General image, standalone sprite, icon, concept that is not a skill icon or item icon | MCP `create_image_pixflux`/`create_image_pixen`/`create_image_pro` + `get_image` when MCP-first (same model choice as REST, minus BitForge and multi-image style reference); REST v2 image generation otherwise. | Route skill/ability/spell/action-bar/hotbar icons through the skill-icon reference, and inventory/equipment/loot/pickup icons through the item-icon reference. |
| Background, scene, title image | REST v2 background/image generation. | Use documented `create-image-pixflux-background` for background-image generation when REST v2 is the selected surface; treat as normal image generation unless the user needs map/editor semantics. A blind single-reviewer benchmark ([results](../pixellab-image-model-benchmark-results.md)) split the model choice: **subject-less backdrops → PixFlux** (beats Pixen), but **full scenes with a subject → Pixen** (beats PixFlux; Pro only third). Pro is not worth its ~12× cost for scenes/backdrops/parallax — its edge is isolated effects, tiny icons, and reference/style work. Two off-label findings from the same run: **Pixen is weakest for standalone objects/props** (prefer PixFlux there), and **BitForge is consistently weakest on raw image quality** — use it only when its direct `coverage_percentage` control is required, not as a default quality pick. Directional evidence (2 seeds, one reviewer), not a decisive ranking. |
...
| Image edit, inpaint, remove background, resize, convert to pixel art | MCP `edit_image` (pro; text instruction or reference-image) for an edit, or `inpaint_image` (rectangular or custom mask) for a masked-region regenerate, when MCP-first. Remove background, resize, and convert-to-pixel-art have no MCP tool; use REST v2 edit/transform endpoints. | Supplied images are optional unless the selected route requires image fields. For image-to-pixel-art without a requested size, prefer Pro. For fixed output size within current `output_size` limits, use normal `image-to-pixelart`. If the requested size is outside those limits, use Pro, verify dimensions, and if they differ, warn the user and ask before using PixelLab `resize` or local nearest-neighbor/canvas resize/pad/crop. |
| Animate an existing image/sprite directly (no managed character/object) | MCP `animate_image` (preferred frame URLs or inline base64, plus `action`; optional last frame for a tween) when MCP-first; REST v2 `animate-with-text-v3` otherwise. | MCP covers the core v3 workflow; REST additionally exposes `drift_threshold` and `enhance_prompt`. |
| Multi-image edit or combine references | MCP `edit_image` when MCP-first (preferred `image_urls`, or inline base64; optional reference URL/base64 switches to reference mode); REST v2 `edit-images-v2` otherwise. | MCP `edit_image` matches the `edit-images-v2` (Pro) workflow, not base `edit-image`. Aseprite's observed `generate-multi-edit` operation is an undocumented internal endpoint, not a public REST route. |
...
| Add VFX/effect/trail/aura to an existing sprite | REST v2 image edit. | When a target sprite/image is supplied, preserve that image and apply the effect to the same canvas. |
| Isolated reusable VFX/effect sprite | Create Image Pro (`generate-image-v2`) first. | Pro is the reliable route found for effect candidates and variety. Focused aura research found Pixen unreliable: it frequently generated characters, scenes, emblems, platforms, central objects, or fully top-down compositions, and its rare good sheets did not repeat consistently. See the [character-aura research spike](pixellab-character-aura-prompt-research-spike.md). |
...
- `tiles`: terrain tileset, sidescroller tileset, isometric tile, tile variants, a connectable path/road set, or a building kit.
