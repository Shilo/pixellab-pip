...
- **PixelArt Workbench:** MCP adds `pixelart_workbench`, an MCP-only command tool for model-authored pixel-art operations. The live tool description says commands are free for subscribers, but a 2026-09-25 Workbench-only test window coincided with a +6 account generation-balance delta and no per-call `usage.generations`. Concurrent account use could not be excluded, so treat its billing as unverified; do not report a confirmed zero-generation cost. The release announcement reports roughly 70% lower token consumption in its testing; this repository did not independently benchmark that estimate.
