...
| `Image to text` / PixelLab prompt from image | `POST /image-to-text` | None documented | PixelLab prompt-description API operation | Converts an image into prompt text; no image is generated | Public REST only | High. Evidence: current OpenAPI and REST llms index. |
...
- Pixel art to a PixelLab-ready prompt -> REST `POST /v2/image-to-text`; this returns text and has no MCP equivalent.
