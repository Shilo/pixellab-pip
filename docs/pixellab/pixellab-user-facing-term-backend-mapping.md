...
- Character: `create_character`, `create_portrait_character`, `get_portrait_character`, `create_character_state`, `animate_character`, `get_character`, `list_characters`, `update_character_tags`, `delete_character`, `delete_animation`.
...
As of the 2026-07-26 refresh, MCP covers the raw-image edit/inpaint family, but against the Pro variants rather than the base ones: `edit_image` matches the `edit-images-v2` (Pro) field shape (`images_base64` list, `reference_image_base64` = reference mode) and `inpaint_image` matches `inpaint-v3` (Pro) (rect or custom mask plus `crop_to_mask`). Base `edit-image` (with `text_guidance_scale`/`color_image`) and base `inpaint` (with `negative_description`, guidance scales, style fields) have no MCP equivalent, and neither do `resize` or `remove-background`; route those to REST v2 unless actual MCP tools are visible in a client.
...
| `Create S-XL image (Pro)` / Aseprite `Create S-XL image (pro)` | `POST /generate-image-v2` | `create_image_pro` + `get_image` (mirrors `reference_images`, `style_image`, `style_options` as `style_image_base64`/`style_copy`) | Aseprite request identifiers include `model_name = "generate_image_new"` and root label `generate-image-new` | Pro image workflow; provider undisclosed | Public REST plus editor wrapper | High for REST mapping, medium for exact website backend. API page lists `/v2/generate-image-v2` and plugin tool `Create S-XL image (Pro)`. Aseprite terminology evidence informs this row; source filenames are intentionally omitted. |
...
| User-facing label | Public REST v2 | MCP tool | First-party label/operation note | Product/model label | Public/internal status | Confidence and evidence |
|---|---|---|---|---|---|---|
| `Animate with text (new)` | `POST /animate-with-text-v3`; helper `POST /enhance-animation-v3-prompt` | `animate_image` for a raw sprite (`first_frame_base64` + `action`, optional `last_frame_base64`); `animate_character`/`animate_object` remain managed-asset only | Aseprite root `animate-with-text-v3`, `model_name = "generate_animate_with_text_v3"` | v3 animation workflow; `new` is UI label | Public REST plus editor wrapper | High. Evidence: API page lists `/v2/animate-with-text-v3`; docs nav says `Animate with text (New)`; Aseprite terminology evidence informs this row; source filenames are intentionally omitted. |
...
| `Inpaint (v3)` / `Inpaint (Pro)` | `POST /inpaint-v3` | `inpaint_image` (rect `mask_x/y/width/height` or `mask_image_base64`, plus `crop_to_mask`) | Aseprite root `generate-inpainting-v3`; file title says `Inpaint (pro)`, menu says `Inpaint (v3)` | v3/Pro inpaint workflow | Public REST plus editor wrapper | High, with label collision. Evidence: API page calls `/v2/inpaint-v3` `Inpaint (Pro)`; docs nav says `Inpaint v3`; Aseprite terminology evidence informs this row; source filenames are intentionally omitted. |
...
| `Edit image (Pro)` / `Edit images (Pro)` | `POST /edit-images-v2` | `edit_image` (`images_base64` list; `reference_image_base64` = the `edit_with_reference` method) | Aseprite root `generate-edit-image-pro`, `model_name = "edit_image_pro"` | Pro image edit, multi/reference-capable | Public REST plus editor wrapper | High. Evidence: API page `/v2/edit-images-v2`; Aseprite terminology evidence informs this row; source filenames are intentionally omitted. |
| `Multi-image edit` / docs `Multi image` | Partial: closest public route `POST /edit-images-v2` | `edit_image` with several `images_base64` (same edit applied across frames, not a multi-source composite) | Aseprite root `generate-multi-edit`, `model_name = "generate_multi_edit"` | Multi-image editor workflow | Editor-specific wrapper with public partial | Medium. Aseprite terminology evidence informs this row; source filenames are intentionally omitted. |
