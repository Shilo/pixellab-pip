...
- Pixel-art command workflow: `pixelart_workbench` operates on PixelLab-managed IDs rather than local paths.
...
Version 0.4.125 adds MCP `pixelart_workbench` and Skeleton v3 in Character Creator, Aseprite, and Pixelorama. The new Skeleton v3 public routes are REST `POST /animate-with-skeleton-v3` and MCP `animate_with_skeleton_v3`; managed characters also expose `animate_character(mode="skeleton-v3")`. REST adds `POST /image-to-text` with no MCP counterpart. The current snapshot has 95 REST paths, 263 schemas, and 110 MCP tools.
...
| `Animate with skeleton (v3)` / Skeleton v3 | `POST /animate-with-skeleton-v3`; managed `POST /animate-character` and `/characters/animations` also accept `mode="skeleton-v3"` | `animate_with_skeleton_v3` for raw keypoints; `animate_character(mode="skeleton-v3")` for managed characters | Version 0.4.125 lists Character Creator, Aseprite, and Pixelorama | Tier 1+ beta; 3–15 pose sets up to 256×256 | Public REST/MCP plus first-party product surfaces | High for public routes/tool; medium for exact editor workflow equivalence. Evidence: current OpenAPI, MCP docs, API page, and the 0.4.125 announcement. |
...
| Skeleton v3 | Keypoint-guided animation workflow distinct from legacy `/animate-with-skeleton` | REST `/animate-with-skeleton-v3` ↔ MCP `animate_with_skeleton_v3`; managed `animate_character(mode="skeleton-v3")` | Product/version, provider undisclosed | High for public routing; beta status and tier from current docs |
...
- Explicit Skeleton v3 animation -> MCP `animate_with_skeleton_v3` or REST `POST /v2/animate-with-skeleton-v3`; managed characters can use `animate_character(mode="skeleton-v3")`.
