...
The Pro Flash family (introduced as Pro Fast) has five matched paid operations and a matched capability lookup, but only REST exposes `/pro-flash/cost`. The name does not establish a price, speed, or quality win over older Pro routes. The official MCP character examples under its heading call the wrong tool and conflict with the eight-only parameter; use the live schema for executable calls.
...
| User-facing label | Public REST v2 | MCP tool | First-party label/operation note | Product/model label | Public/internal status | Confidence and evidence |
|---|---|---|---|---|---|---|
| `Map Workshop` / `Maps` | Still no public REST map CRUD; REST map-adjacent endpoints remain tilesets, tiles pro, isometric tiles, map objects | Full map CRUD as of 2026-09-08: `create_map`, `list_maps`, `get_map`, `view_map`, `edit_map`, `delete_map`, plus `place_map_object`, `move_map_object`, `remove_map_object`, `list_map_objects` | Website `/maps` chunk uses `/maps/save`, `/maps/list`, `/maps`, `/maps/tiles`, imports, exports; page says Map Workshop | Website map editor | Public MCP surface; website editor internals remain internal | High. Evidence: MCP docs map tool section; `https://www.pixellab.ai/maps` says Map Workshop requires subscription; current maps chunk exposes `/maps/save` and `/maps/list`. |
