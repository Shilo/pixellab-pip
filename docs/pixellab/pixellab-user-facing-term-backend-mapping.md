...
| `Create images from style references (Pro)` / `Generate with style (Pro)` | `POST /generate-with-style-v2` (requires `style_images`, plural) | No multi-style-image equivalent; `create_image_pro` takes one style image URL/base64 plus `style_copy` | Aseprite request identifiers include `model_name = "generate_consistent_style"` and root label `generate-consistent-style` | Pro style-reference workflow | Public REST plus editor wrapper | High for public route, medium for exact wrapper parity. |
