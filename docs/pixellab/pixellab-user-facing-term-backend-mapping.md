...
- Images and UI: `/generate-image-v2`, `/generate-with-style-v2`, `/generate-ui-v2`, `/create-ui-asset`, `/ui-assets`, `/create-image-pixflux`, `/create-image-pixflux-background`, `/create-image-pixen`, `/create-image-bitforge`, `/create-image-pro-flash`, `/pro-flash/capabilities`, `/pro-flash/cost`.
- Image operations: `/image-to-pixelart`, `/image-to-text`, `/image-to-pixelart-pro`, `/resize`, `/remove-background`, `/edit-image`, `/edit-image-pixen`, `/edit-images-v2`, `/inpaint`, `/inpaint-v3`, `/edit-image-pro-flash`, `/inpaint-image-pro-flash`.
- Cleanup: `/correct-pixelart`, `/reduce-colors`, `/unzoom`.
- Animation and rotation: `/animate-with-text`, `/animate-with-text-v2`, `/animate-with-text-v3`, `/animate-pixminimax`, `/animate-with-skeleton-v3`, `/animate-with-skeleton`, `/estimate-skeleton`, `/edit-animation-v2`, `/interpolation-v2`, `/transfer-outfit-v2`, `/rotate`, `/generate-8-rotations-v2`, `/generate-8-rotations-v3`.
- Characters: `/create-character-with-4-directions`, `/create-character-with-8-directions`, `/create-character-pro`, `/create-character-v3`, `/create-character-pro-flash`, `/portrait-character-pro`, `/characters/{character_id}/portrait`, `/vocal-animation`, `/talking-gif`, `/lip-sync`, `/create-character-state`, `/animate-character`, `/characters/animations`, plus character list/get/delete/tag/zip/spritesheet.
- Fonts: `/generate-font-pro`.
- Objects: `/create-1-direction-object`, `/create-8-direction-object`, `/create-object-pro-flash`, `/objects/{object_id}/animations`, `/objects/{object_id}/states`, review selection, list/get/delete/tag/spritesheet.
...
Not found in public v2 OpenAPI: map CRUD, Map Workshop map export/import (including the Godot/Unity export surface), Pixelorama/editor `replace-png`, try-on image, reshape, create texture, raw website `/tilesets/create`, raw website `/tiles/create`, or Aseprite root `generate-*` operation paths. Reduce colors/quantize, unzoom, and pixel correction left this list in the 2026-09-08 refresh — they are now public REST v2 endpoints. Map CRUD is still absent from REST but exists on MCP (see the Map Workshop row below).
...
- Character and talking portrait: `create_character`, `create_character_pro_flash`, `create_portrait_character`, `get_portrait_character`, `set_character_portrait`, `create_vocal_animation`, `get_vocal_animation`, `create_talking_gif`, `get_lip_sync`, `create_character_state`, `animate_character`, `get_character`, `list_characters`, `update_character_tags`, `delete_character`, `delete_animation`.
...
- Objects and map objects: `create_map_object`, `get_map_object`, `create_1_direction_object`, `create_8_direction_object`, `create_object_pro_flash`, `get_object`, `list_objects`, `animate_object`, `create_object_state`, `update_object_tags`, `delete_object`, `select_object_frames`, `dismiss_review`.
- UI assets: `create_ui_asset`, `get_ui_asset`, `list_ui_assets`, `delete_ui_asset`.
- Platform helpers: `get_balance`, `list_projects`, `add_to_project`, `remove_from_project`, `chat_*`, `sandbox_*`, `agent_*`, `agent_feedback`, `agent_help`, `search_knowledge`, `list_jobs`, `wait_for_jobs`, `cancel_job`.
...
- Raw image (no managed asset): `create_image_pixflux`, `create_image_pixen`, `create_image_pro`, `create_image_pro_flash`, `get_image`, `edit_image`, `edit_image_pixen`, `edit_image_pro_flash`, `inpaint_image`, `inpaint_image_pro_flash`, `animate_image`, `animate_image_pixminimax`, `animate_with_skeleton_v3`, `image_to_pixelart`, `unzoom_image`, `correct_pixelart`, `reduce_colors`; Pro Flash size lookup: `get_pro_flash_capabilities`.
- Pixel-art command workflow: MCP-only `pixelart_workbench` accepts PixelLab IDs and supported image references, not local paths; see the [Workbench reference](../../skills/pixellab-pip/references/pixelart-workbench.md).
- Path tiles and building kits (share the tiles-pro get/list/delete tools): `create_path_tiles`, `create_building_kit`.

As of the 2026-09-25 refresh, MCP covers the raw-image edit/inpaint family against the Pro variants and accepts preferred URL inputs as alternatives to inline base64. `edit_image_pixen` is the 1-generation Pixen edit, matching REST `edit-image-pixen`; `animate_image_pixminimax` matches PixMiniMax raw animation; and `animate_with_skeleton_v3` matches Skeleton v3. Base `edit-image` and base `inpaint` retain REST-only controls, and `image-to-text`, `image-to-pixelart-pro`, `resize`, and `remove-background` still have no MCP tool. `pixelart_workbench` is MCP-only and has no REST route. Talking-portrait workflows have counterparts on both surfaces except REST's stateless `/lip-sync` form, because MCP `get_lip_sync` requires a managed character.

The Pro Flash family has five matched paid operations and a matched capability lookup, but only REST exposes `/pro-flash/cost`. Version 0.4.123 advertises its single-image beta option at 4–6 generations and up to `256x256`; current REST OpenAPI describes a provisional five-generation first-image estimate. The name does not establish a price, speed, or quality win over older Pro routes. The official MCP character examples under its heading call the wrong tool and conflict with the eight-only parameter; use the live schema for executable calls.

Version 0.4.123 also makes PixMiniMax available to Tier 1+ subscribers in Character Creator, Creator, Aseprite, and Pixelorama, and makes Game Builder available to Tier 1+. These are first-party product surfaces. The public programmatic PixMiniMax mapping remains REST `POST /animate-pixminimax` and MCP `animate_image_pixminimax`; no dedicated REST v2 Game Builder endpoint is documented.

Version 0.4.125 adds MCP `pixelart_workbench` and Skeleton v3 in Character Creator, Aseprite, and Pixelorama. The new Skeleton v3 public routes are REST `POST /animate-with-skeleton-v3` and MCP `animate_with_skeleton_v3`; managed characters also support `animate_character(mode="skeleton-v3", template_animation_id=...)`. REST adds `POST /image-to-text` with no MCP counterpart. The current snapshot has 95 REST paths, 263 schemas, and 110 MCP tools.
...
|---|---|---|---|---|---|---|
| `Create image S-XL (new)` / `Create Image S-XL (New)` | `POST /create-image-pixen`; `POST /enhance-pixen-prompt` for prompts | `create_image_pixen` + `get_image` | Aseprite menu and request identifiers include `generate_pixen`, `model_name = "pixen"`, and root label `generate-image-pixen` | Pixen; `new` is UI/status; `S-XL` is size/tool label | Public REST plus editor wrapper | High. API page lists `Create image (Pixen)` at `/v2/create-image-pixen` and plugin tool `Create image S-XL (new)`. Aseprite terminology evidence informs this row; source filenames are intentionally omitted. |
| `Create M-XL image` / `Create M-XL image (new)` | `POST /create-image-pixflux`; background variant `POST /create-image-pixflux-background` | `create_image_pixflux` + `get_image` (no separate background variant; use `no_background=False`) | Aseprite menu and request identifiers include `flux`, `model_name = "generate_pixelart_flux"`, and root label `generate-pixelart-flux`; website/API page says plugin tool `Create M-XL image (new)` | PixFlux/Pixflux; `M-XL` is size/tool label | Public REST plus editor wrapper | High for PixFlux. Aseprite terminology evidence informs this row; source filenames are intentionally omitted. |
| `Create S-XL image (Pro)` / Aseprite `Create S-XL image (pro)` | `POST /generate-image-v2` | `create_image_pro` + `get_image` (labelled references and one style image; URLs preferred on MCP) | Aseprite request identifiers include `model_name = "generate_image_new"` and root label `generate-image-new` | Pro image workflow; provider undisclosed | Public REST plus editor wrapper | High for REST mapping, medium for exact website backend. |
| `Create image (Pro Flash)` | `POST /create-image-pro-flash`; `/pro-flash/capabilities` and `/pro-flash/cost` for size/price checks | `create_image_pro_flash`, `get_pro_flash_capabilities` | No matching website/editor label established in this review | Separate one-image Pro Flash workflow | Public REST/MCP; REST-only cost lookup | High for route names/fields, untested for output quality and speed. |
| Docs nav `Create S-L image (Pro)` | Same as above by current link target: `/docs/tools/create-sl-image-pro`, API page labels S-XL | `create_image_pro`, same as the row above | No distinct Aseprite `S-L` tool found | Likely docs/navigation mismatch | Ambiguous public docs label | Medium. `https://www.pixellab.ai/docs` nav says `Create S-L image (Pro)`, while the API page and Aseprite label say S-XL. Treat `S-L` as alias only after verifying page contents. |
...
| `Create images from style references (Pro)` / `Generate with style (Pro)` | `POST /generate-with-style-v2` (requires 1–4 `style_images`; output size is derived from their largest dimension, not a supported `image_size` field) | No multi-style-image equivalent; `create_image_pro` takes one style image URL/base64 plus `style_copy` | Aseprite request identifiers include `model_name = "generate_consistent_style"` and root label `generate-consistent-style` | Pro style-reference workflow | Public REST plus editor wrapper | High for public route, medium for exact wrapper parity. |
...
| `Image to pixel art` | `POST /image-to-pixelart` | `image_to_pixelart` (MCP `faithful` is REST `fixer`) | Aseprite root `generate-image-to-pixelart` | Converter | Public REST plus editor wrapper | High. Evidence: API page `/v2/image-to-pixelart`; Aseprite terminology evidence informs this row; source filenames are intentionally omitted. |
| `Image to text` / image caption or visual question | `POST /image-to-text` (one image, optional custom `prompt`) | None documented | PixelLab image-reading API operation | Default generic generation description or custom image-grounded answer; not target-specific prompt enhancement and no image is generated | Public REST only | High. Current OpenAPI; see [research spike](pixellab-image-to-text-research-spike.md) for enhancer comparison. |
| `PixelArt Workbench` | No public REST route | `pixelart_workbench` | Version 0.4.125 MCP release feature | Model-authored pixel-level draw, edit, inspect, and animation commands | Public MCP-only tool | High for tool existence and command contract; the release's token-saving estimate is not independently measured here. |
...
| `Animate with text (PixMiniMax)` | `POST /animate-pixminimax`; helper `POST /enhance-animation-v3-prompt` with `engine="pixminimax"`; inline `enhance_prompt`, `direction`, and `drift_threshold` are route-specific | `animate_image_pixminimax` for a raw sprite (preferred frame URLs or inline base64) | Version 0.4.123 lists Character Creator, Creator, Aseprite, and Pixelorama as human surfaces; private editor transport is not used as the public mapping | PixMiniMax; public REST description says powered by MiniMax H3; Tier 1+ availability | Public REST/MCP plus first-party product/editor surfaces | High for the public route/tool; medium for exact editor parity. Evidence: current OpenAPI, MCP docs, API page, the 0.4.123 product notes, and local terminology inspection. |
...
| `Animate with skeleton (v3)` / Skeleton v3 | `POST /animate-with-skeleton-v3`; managed `POST /animate-character` and `/characters/animations` also accept `mode="skeleton-v3"` with `template_animation_id` | `animate_with_skeleton_v3` for raw keypoints; managed `animate_character(mode="skeleton-v3", template_animation_id=...)` when exposed by the connected schema | Version 0.4.125 lists Character Creator, Aseprite, and Pixelorama | Tier 1+ beta; raw route takes 3–15 pose sets up to 256×256; managed route uses a named template | Public REST/MCP plus first-party product surfaces | High for public routes/tool; medium for exact editor workflow equivalence. Evidence: current OpenAPI, MCP docs, API page, and the 0.4.125 announcement. |
| `Animate with skeleton` (legacy) | `POST /animate-with-skeleton`; `POST /estimate-skeleton` for pose utility | No raw MCP equivalent for this legacy payload | Aseprite root `generate-pose-animation` or `generate-animation`; newer menu label `Animate with skeleton (new)` appears in the extension | Older three-pose skeleton animation workflow | Public REST plus editor wrappers | Medium. Evidence: API llms lists both legacy endpoints; Aseprite terminology evidence informs this row; source filenames are intentionally omitted. |
| `Interpolate (new)` | No separately named v3 interpolation path; the v3 tween is `POST /animate-with-text-v3` with `last_frame` set. The named Pro route is `POST /interpolation-v2` | `animate_image` with `last_frame_base64` | Aseprite root `animate-with-text-v3`, `model_name = "generate_interpolation_v3"` | New/v3 editor workflow | Editor-specific wrapper with public partial | Low/medium. Aseprite terminology evidence informs this row; source filenames are intentionally omitted. |
...
| `Create character (Pro)` | `POST /create-character-pro` | `create_character(mode="pro")` — live schema names the mode explicitly (20-40 generations, always 8 directions); exact internal route still hidden | Website/REST docs only; Aseprite Pro sprite rotate is not necessarily this endpoint | Pro character workflow | Public REST; MCP capability mapping inferred | Medium/high. Evidence: API page `/v2/create-character-pro`; MCP `create_character` has `mode`. |
| `Create character (Pro Flash)` | `POST /create-character-pro-flash` | `create_character_pro_flash` | No matching website/editor label established | First image plus eight V3 views; can reuse an owned south-facing image | Public REST/MCP | High for route/schema, untested for visual quality. The MCP page's copied example uses the wrong tool and its four-direction hint is invalid. |
| `Create character v3` | `POST /create-character-v3`; helper `/enhance-character-v3-prompt` | `create_character(mode="v3")` — the only mode accepting `reference_image_base64`/`reference_image_url`; exact internal route still hidden | Website docs; no exact Aseprite label found besides v3-adjacent rotation/animation | v3 character model/workflow | Public REST; MCP capability mapping inferred | High for REST, medium for MCP exact route. Evidence: API page says create an 8-direction character using v3 model. |
...
| `Set character portrait` | `POST /characters/{character_id}/portrait` | `set_character_portrait` | Stores/replaces the portrait used by vocal animation | Managed character portrait | Public REST/MCP | High. Current OpenAPI and MCP docs agree; replacement is free but destructive. |
| `Vocal animation` / mouth shapes / visemes | `POST /vocal-animation`, then `GET /vocal-animation/{job_id}` | `create_vocal_animation`, `get_vocal_animation` | One paid async call per mood; use the dedicated getter | Talking portrait generation | Public REST/MCP | High. Current OpenAPI, llms, and MCP docs agree. |
| `Talking GIF` | `POST /talking-gif` | `create_talking_gif` | Free synchronous renderer from dialogue and stored/supplied visemes | Talking portrait output | Public REST/MCP | High. Current OpenAPI, llms, and MCP docs agree. |
| `Lip sync` / viseme timing plan | `POST /lip-sync` | `get_lip_sync` for a managed character | REST additionally supports stateless `viseme_count` | Talking portrait timing | Public REST/MCP partial | High. Preserve dialogue exactly; Latin-alphabet text is documented. |
| `Create Object` website page | `POST /create-1-direction-object`, `/create-8-direction-object`, object state/animation/review endpoints | `create_1_direction_object`, `create_8_direction_object`, `get_object`, `list_objects`, `animate_object`, `create_object_state`, `update_object_tags`, `select_object_frames`, `dismiss_review`, `delete_object` | Website `/create-object` chunk uses root `/objects/zip`, `/objects/delete-batch`; page button says `Create Object` | Object workflow | Public REST/MCP for core; website batch routes internal | High for public, medium for website batch helpers. Evidence: API llms object section; MCP docs object tools; website chunk `create-object-94d4405f1f185485.js` route snippets. |
| `Create object (Pro Flash)` | `POST /create-object-pro-flash` | `create_object_pro_flash` | No matching website/editor label established | One or eight directions; can reuse an owned south-facing image | Public REST/MCP | High for route/schema, untested for visual quality. |
...
| `Create map object` / `Map Objects` | `POST /map-objects` | `create_map_object`, `get_map_object`, then `place_map_object` to put it on a map | Website Map Workshop object flows are root map/editor routes | Map object generation | Public REST/MCP for generated map object; website map editing internal | High. Evidence: API page `/v2/map-objects`; MCP docs, which as of 2026-09-08 state the object is permanent (the previous 8-hour auto-delete note is gone). |

### Maps, Tiles, Map Workshop, And Pixelorama

| User-facing label | Public REST v2 | MCP tool | First-party label/operation note | Product/model label | Public/internal status | Confidence and evidence |
|---|---|---|---|---|---|---|
| `Map Workshop` / `Maps` | Still no public REST map CRUD or map-export route; REST map-adjacent endpoints remain tilesets, tiles pro, isometric tiles, map objects | Full map CRUD as of 2026-09-08: `create_map`, `list_maps`, `get_map`, `view_map`, `edit_map`, `delete_map`, plus `place_map_object`, `move_map_object`, `remove_map_object`, `list_map_objects` | Website `/maps` chunk uses `/maps/save`, `/maps/list`, `/maps`, `/maps/tiles`, imports, exports; Version 0.4.123 announces Godot and Unity export; page says Map Workshop | Website map editor/export | Public MCP map surface; website editor/export internals remain internal | High for public MCP and website surface; medium for exact export options. Evidence: MCP docs map tool section, current `/maps` page/chunk, and the 0.4.123 product notes. |
| `Game Builder` | No dedicated public REST v2 endpoint | Public project/chat/sandbox helpers such as `list_projects`, `add_to_project`, `chat_*`, and `sandbox_*` may provide approved project context; no dedicated Game Builder MCP tool is documented | First-party Game Builder product; official 0.4.123 notes make it available to Tier 1+ subscribers | Game-building workflow | First-party product with public MCP platform context; not a REST contract | Medium. Product availability comes from the 0.4.123 notes; helper availability comes from current MCP docs; exact internal linkage is not documented. |
...
| `Inpaint (v3)` / `Inpaint (Pro)` | `POST /inpaint-v3` | `inpaint_image` (preferred target/mask URLs or inline base64; rectangular masks also supported; plus `crop_to_mask`) | Aseprite root `generate-inpainting-v3`; file title says `Inpaint (pro)`, menu says `Inpaint (v3)` | v3/Pro inpaint workflow | Public REST plus editor wrapper | High, with label collision. Evidence: API page calls `/v2/inpaint-v3` `Inpaint (Pro)`; docs nav says `Inpaint v3`; Aseprite terminology evidence informs this row; source filenames are intentionally omitted. |
| `Inpaint image (Pro Flash)` | `POST /inpaint-image-pro-flash` | `inpaint_image_pro_flash` | No matching website/editor label established | Separate native-size masked-edit workflow | Public REST/MCP | High for route/schema, outside-mask byte-preservation claim untested. |
...
| `Edit image (Pro)` / `Edit images (Pro)` | `POST /edit-images-v2` | `edit_image` (preferred target/reference URLs or inline base64) | Aseprite root `generate-edit-image-pro`, `model_name = "edit_image_pro"` | Pro image edit, multi/reference-capable | Public REST plus editor wrapper | High. |
| `Edit image (Pro Flash)` | `POST /edit-image-pro-flash` | `edit_image_pro_flash` | No matching website/editor label established | Separate one-image edit at unchanged native canvas | Public REST/MCP | High for route/schema, untested for output quality. |
| `Multi-image edit` / docs `Multi image` | Partial: closest public route `POST /edit-images-v2` | `edit_image` with several target URLs/base64 images (same edit across frames, not a multi-source composite) | Aseprite root `generate-multi-edit`, `model_name = "generate_multi_edit"` | Multi-image editor workflow | Editor-specific wrapper with public partial | Medium. |
...
| `Reduce colors` / `Quantize Colors` | `POST /reduce-colors` | `reduce_colors` | Aseprite root `quantize-image`; dialog title `Quantize Colors | Reduce colors` | Quantization utility | Public REST/MCP plus editor wrapper | High. Evidence: OpenAPI `/v2/reduce-colors` and MCP docs `reduce_colors`, both first seen in the 2026-09-08 refresh; Aseprite terminology evidence informs this row; source filenames are intentionally omitted. |
| `Unzoom pixel art` | `POST /unzoom` | `unzoom_image` | Aseprite root `unzoom-pixelart`, `model_name = "unzoom_pixelart"` | Unzoom utility | Public REST/MCP plus editor wrapper | High. Evidence: OpenAPI `/v2/unzoom` and MCP docs `unzoom_image`, both first seen in the 2026-09-08 refresh; Aseprite terminology evidence informs this row; source filenames are intentionally omitted. |
| `Pixel correction` | `POST /correct-pixelart` | `correct_pixelart` | Aseprite root `correct-pixelart`, `model_name = "generate_correct_pixelart"` | Pixel correction utility | Public REST/MCP plus editor wrapper | High. Evidence: OpenAPI `/v2/correct-pixelart` and MCP docs `correct_pixelart`, both first seen in the 2026-09-08 refresh; Aseprite terminology evidence informs this row; source filenames are intentionally omitted. |
...
| PixelArt Workbench | MCP command tool for model-authored pixel operations | MCP `pixelart_workbench`; no REST route | PixelLab feature/tool label; no provider disclosure | High for MCP contract; token-saving estimate is an announcement claim |
...
| Pro Flash | Separate beta one-image/character/object/edit/inpaint family, not an alias for older Pro | Five `*-pro-flash` REST paths and six MCP tools including capability lookup; REST `/pro-flash/cost` has no MCP tool counterpart; single-image notes advertise 4–6 generations up to `256x256`, while OpenAPI says five provisionally | Product family, provider undisclosed | High for public route existence; no live quality or cost test |
...
| Skeleton v3 | Keypoint-guided raw animation or managed named-template animation, distinct from legacy `/animate-with-skeleton` | REST `/animate-with-skeleton-v3` ↔ MCP `animate_with_skeleton_v3`; managed `animate_character(mode="skeleton-v3", template_animation_id=...)` when the connected schema exposes the mode, otherwise REST `/animate-character` with both `mode="skeleton-v3"` and `template_animation_id` | Product/version, provider undisclosed | High for public routing; beta status and tier from current docs |
...
6. `editor-only-utilities.md`
   ...
   - Contents: try-on, reshape, create texture, Canny/pose/depth distinctions. (Reduce colors/quantize, unzoom, and pixel correction moved to public REST/MCP routes in the 2026-09-08 refresh and no longer belong here.)
...
- Explicit Pro Flash -> its separate `*-pro-flash` REST path or MCP tool; check size/cost and do not infer a quality or speed advantage.
...
- Pixel-level model-authored operations -> MCP `pixelart_workbench`; it has no REST endpoint.
- PixelLab-generated caption or visual answer from an image -> REST `POST /v2/image-to-text`; it returns text and has no MCP equivalent. For enhancing existing text for a known generator, use the matching `enhance-*` route instead.
- Animation new/v3 -> `POST /v2/animate-with-text-v3`.
- Explicit raw Skeleton v3 animation -> MCP `animate_with_skeleton_v3` or REST `POST /v2/animate-with-skeleton-v3`; managed characters use `animate_character(mode="skeleton-v3", template_animation_id=...)` when the connected schema exposes the mode, otherwise REST `POST /animate-character` with `mode="skeleton-v3"` and `template_animation_id`.
