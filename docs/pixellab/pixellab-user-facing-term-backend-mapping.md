...
- Images and UI: `/generate-image-v2`, `/generate-with-style-v2`, `/generate-ui-v2`, `/create-ui-asset`, `/ui-assets`, `/create-image-pixflux`, `/create-image-pixflux-background`, `/create-image-pixen`, `/create-image-bitforge`, `/create-image-pro-fast`, `/pro-fast/capabilities`, `/pro-fast/cost`.
- Image operations: `/image-to-pixelart`, `/image-to-pixelart-pro`, `/resize`, `/remove-background`, `/edit-image`, `/edit-image-pixen`, `/edit-images-v2`, `/inpaint`, `/inpaint-v3`, `/edit-image-pro-fast`, `/inpaint-image-pro-fast`.
...
- Characters: `/create-character-with-4-directions`, `/create-character-with-8-directions`, `/create-character-pro`, `/create-character-v3`, `/create-character-pro-fast`, `/portrait-character-pro`, `/characters/{character_id}/portrait`, `/vocal-animation`, `/talking-gif`, `/lip-sync`, `/create-character-state`, `/animate-character`, `/characters/animations`, plus character list/get/delete/tag/zip/spritesheet.
- Fonts: `/generate-font-pro`.
- Objects: `/create-1-direction-object`, `/create-8-direction-object`, `/create-object-pro-fast`, `/objects/{object_id}/animations`, `/objects/{object_id}/states`, review selection, list/get/delete/tag/spritesheet.
...
- Character and talking portrait: `create_character`, `create_character_pro_fast`, `create_portrait_character`, `get_portrait_character`, `set_character_portrait`, `create_vocal_animation`, `get_vocal_animation`, `create_talking_gif`, `get_lip_sync`, `create_character_state`, `animate_character`, `get_character`, `list_characters`, `update_character_tags`, `delete_character`, `delete_animation`.
...
- Objects and map objects: `create_map_object`, `get_map_object`, `create_1_direction_object`, `create_8_direction_object`, `create_object_pro_fast`, `get_object`, `list_objects`, `animate_object`, `create_object_state`, `update_object_tags`, `delete_object`, `select_object_frames`, `dismiss_review`.
...
- Raw image (no managed asset): `create_image_pixflux`, `create_image_pixen`, `create_image_pro`, `create_image_pro_fast`, `get_image`, `edit_image`, `edit_image_pixen`, `edit_image_pro_fast`, `inpaint_image`, `inpaint_image_pro_fast`, `animate_image`, `animate_image_pixminimax`, `image_to_pixelart`, `unzoom_image`, `correct_pixelart`, `reduce_colors`; Pro Fast size lookup: `get_pro_fast_capabilities`.
...
The later Pro Fast addition has five matched paid operations and a matched capability lookup, but only REST exposes `/pro-fast/cost`. The name does not establish a price, speed, or quality win over older Pro routes. The official MCP character examples under the new heading call the wrong tool and conflict with the eight-only parameter; use the live schema for executable calls.
...
| `Create image (Pro Fast)` | `POST /create-image-pro-fast`; `/pro-fast/capabilities` and `/pro-fast/cost` for size/price checks | `create_image_pro_fast`, `get_pro_fast_capabilities` | No matching website/editor label established in this review | Separate one-image Pro Fast workflow | Public REST/MCP; REST-only cost lookup | High for route names/fields, untested for output quality and speed. |
...
| `Create object (Pro Fast)` | `POST /create-object-pro-fast` | `create_object_pro_fast` | No matching website/editor label established | One or eight directions; can reuse an owned south-facing image | Public REST/MCP | High for route/schema, untested for visual quality. |
...
| `Inpaint image (Pro Fast)` | `POST /inpaint-image-pro-fast` | `inpaint_image_pro_fast` | No matching website/editor label established | Separate native-size masked-edit workflow | Public REST/MCP | High for route/schema, outside-mask byte-preservation claim untested. |
...
| `Edit image (Pro Fast)` | `POST /edit-image-pro-fast` | `edit_image_pro_fast` | No matching website/editor label established | Separate one-image edit at unchanged native canvas | Public REST/MCP | High for route/schema, untested for output quality. |
...
| Pro Fast | Separate one-image/character/object/edit/inpaint family, not an alias for older Pro | Five `*-pro-fast` REST paths and six MCP tools including capability lookup; REST `/pro-fast/cost` has no MCP tool counterpart | Product family, provider undisclosed | High for public route existence; no live quality or cost test |
...
- Explicit Pro Fast -> its separate `*-pro-fast` REST path or MCP tool; check size/cost and do not infer a quality or speed advantage.
