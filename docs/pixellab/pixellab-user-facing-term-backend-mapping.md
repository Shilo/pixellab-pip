...
As of the 2026-08-06 refresh, MCP covers the raw-image edit/inpaint family against the Pro variants and accepts preferred URL inputs as alternatives to inline base64. Base `edit-image` and base `inpaint` retain REST-only controls, and neither `resize` nor `remove-background` has an MCP tool. Talking-portrait workflows have counterparts on both surfaces except REST's stateless `/lip-sync` form, because MCP `get_lip_sync` requires a managed character.
...
- MCP-first raw image work -> `create_image_pixflux` / `create_image_pixen` / `create_image_pro` + `get_image`, then `edit_image`, `inpaint_image`, or `animate_image`. BitForge and multi-style-image generation stay REST-only.
