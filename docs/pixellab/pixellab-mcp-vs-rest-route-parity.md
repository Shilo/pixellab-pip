...
> 2026-09-12 Pro Fast follow-up: REST v2 grew from 86 to 93 paths and 246 to 258 schemas; MCP grew from 100 to 106 tools. Five matched art operations now cover Pro Fast image creation, character creation, object creation, one-image edit, and masked edit; `get_pro_fast_capabilities` matches REST `GET /pro-fast/capabilities`. REST alone exposes `GET /pro-fast/cost`, a provisional price estimator, which is a non-asset infrastructure gap rather than a missing art operation. The asset-gap count remains 25 and the MCP-only count remains 35. The new family is not interchangeable with older Pro: image creation yields one image, character creation always eight directions, object creation one or eight, and edits keep native dimensions. The MCP page's new character examples mistakenly invoke `create_character` with unrelated fields and its four-direction tip conflicts with the eight-only parameter; the REST OpenAPI and live tool schema should decide exact calls. No paid generation was used to validate visual quality or exact-mask behavior.
...
MCP is a managed-asset tool layer inside an agent that also exposes fifteen raw-image primitives (including Pro Fast and the Cleanup family) plus talking-portrait helpers. REST v2 remains the complete HTTP API for the remaining model/version choices, stateless lip sync, and code control.
...
| `POST /create-character-pro-fast` | `create_character_pro_fast` — eight views; MCP adds first-frame URL while both accept an owned source-image ID | =, core workflow |
...
| `POST /create-object-pro-fast` | `create_object_pro_fast` — one or eight views; MCP adds first-frame URL | =, core workflow |
...
MCP's model-specific raw-image tools mirror REST's PixFlux, Pixen, Pro, and separate Pro Fast routes. Their generation controls provide functional parity, while MCP additionally accepts preferred URL forms for PixFlux init/color images and Pro reference/style images. REST retains a few route-specific extras such as `background_removal_task`, deprecated `negative_description`, and `enhance_prompt`. `POST /create-image-pixflux-background` still uses the same `CreateImagePixfluxRequest` as `/create-image-pixflux`, so one MCP tool covers both URLs. Pro Fast makes exactly one image rather than the older Pro route's size-dependent batch.
...
| `POST /create-image-pro-fast` (one image, native size) | `create_image_pro_fast` + raw-image job retrieval | =, core workflow |
| `GET /pro-fast/capabilities` (supported native sizes/controls) | `get_pro_fast_capabilities` | =, core workflow |
...
| `POST /edit-image-pro-fast` (one image, unchanged native canvas) | `edit_image_pro_fast` — MCP additionally accepts an image URL or owned `source_image_id`; REST requires encoded `image` | =, core workflow |
...
| `POST /inpaint-image-pro-fast` (white/black same-size mask; claims exact unmasked RGBA) | `inpaint_image_pro_fast` — MCP adds source/mask URLs, an owned source ID, and rectangular-mask shorthand; REST requires encoded source and mask | =, core workflow; exact-mask claim untested |
...
| `GET /pro-fast/cost` | — no documented MCP cost-estimator tool; REST provides a provisional estimate by operation, size, and direction count | none, non-asset |
...
- `GET /pro-fast/cost` — REST's provisional operation/size/direction estimate. MCP has a capability lookup but no documented cost-estimator tool; actual charged usage belongs to the completed job.
...
- Pro Fast adds a fourth overlap zone for one-image creation, character/object rotations, editing, and masked editing. It is not a new default: prices are provisional, no quality test was run, and the exact-mask promise has not been verified. Check native dimensions with the capability lookup and estimate via REST `/pro-fast/cost` before seeking approval; use completed-job usage as the charge.
...
- MCP↔REST are not guaranteed pixel-identical for the same prompt/seed even where parity is `=`; treat them as one workflow family with overlapping controls, not field-for-field twins. REST generally exposes the fuller documented schema, but Pro Fast MCP adds URL/source-ID inputs while REST alone documents the cost estimator. Pro Fast's image provider explicitly does not promise deterministic output from a recorded seed.
