...
- **BitForge** (`create-image-bitforge` · Aseprite "Create S-M image") — **weakest on raw quality across the board**; every other model beats it. Reach for it only for its **unique controls** — `init_image`, `mask_image`/inpaint, `color_image` (forced palette), `coverage_percentage` (the only route that sets subject-fill directly), `style_image` — never as a default quality pick.
...
- **BitForge** is **consistently the weakest on raw quality** (last or near-last in C1/C2/C4/C5). Its value is its **unique capabilities** — `init_image`, `mask_image`/inpainting, `color_image` (forced palette), `coverage_percentage`, `style_image` — not out-of-box quality. Never pick it as a default quality play.
