...
Directional guidance from this benchmark (static images only). Not a hard rule — useful context when picking a model. A light blind style-tag pass ([style fingerprint](pixellab-image-model-style-fingerprint.md)) corroborates these profiles (lower-weight, anchored pass).

- **Pixen** (`create-image-pixen` · Aseprite "Create Image S-XL (New, Pixen)") — **full scenes** (subject + environment; outright winner), which carry an authentic **"in-game" look**: its scenes and backdrops read like a real playable game screen rather than concept art or an illustration — much of why it took scenes (for *subject-less* backdrops the same quality shows, but PixFlux/Pro still edged it in the blind ranking). Also strong for **characters** (its go-to in practice; benchmarked as best *value*, with Pro marginally ahead on raw quality at ~12× cost). **Signature trait — high detail by default** (`detail` defaults to "highly detailed"), for better or worse: it gives richer characters and more expressive, exaggerated animation, but **can go overboard**, over-shading small or intentionally flat art. So for clean tiny **icons/items**, force `detail: low detail` + a clamped palette, or use Pro; it is also the **weakest model for standalone objects**. That subject bias also makes Pixen **unreliable for isolated reusable VFX**: focused aura testing repeatedly drifted into characters, scenes, emblems, portals, platforms, central objects, or fully top-down effects. *(The Pixen / "New" family is the practitioner pick for characters and their animation — the same high-detail bias that helps exaggerated motion — though animation quality itself was not measured by this static-image benchmark.)*
- **Pro** (`generate-image-v2` · Aseprite "Create S-XL image (pro)") — **isolated reusable VFX**, **small icons & items** (dominant, especially ≤32px via its many candidates), and **style-reference / variety** work. Focused aura testing found Pro reliably produced reviewable modular effect candidates where Pixen did not. It also renders **atmospheric lighting and skies** best — the reason it topped raw quality on subject-less backdrops (C4; e.g. dawn / golden-hour skies) — though PixFlux stays the value pick for backdrops and Pixen still wins full scenes (Pro was only 3rd on C5). The expensive (~12×) option: worth it where it clearly wins — isolated effects, icons/items, style/variety, or a high-quality atmospheric sky/lighting backdrop when budget allows — otherwise skip it for full scenes and parallax.
- **PixFlux** (`create-image-pixflux` · Aseprite "Create M-XL image") — **standalone objects/props** (ties Pro at 1/12 the cost) and **subject-less backdrops** (best value; beats Pixen). Also a co-best pick for **parallax bands** (tied with Pixen). **Signature trait — lower detail** (the inverse of Pixen), for better or worse: a softer, more illustrative render that leans **painterly / stylized / abstract** and frames loose — well suited to scenery, objects, and stylized/abstract art, but too sparse to read cleanly at tiny icon sizes (part of why it is weakest for items). Push its `outline` / `shading` / `detail` controls up when you need more crispness or structure. *(This painted **aesthetic** is distinct from style-**reference** / style-transfer work — matching a supplied style image is Pro's domain; see C7.)*
- **BitForge** (`create-image-bitforge` · Aseprite "Create S-M image") — **weakest on raw quality across the board**; every other model beats it. Use it only when `coverage_percentage` is required; that direct subject-fill control is its remaining unique routing reason. Init/color guidance and stronger edit, inpaint, and style workflows exist on other routes, so those fields alone do not justify BitForge.
...
**Only sometimes.** Clear yes: **isolated reusable VFX** in the later focused aura comparison, **tiny icons (C2)** where its candidate spread dominates, and **style/reference work (C7)**. Clear no: **scenes (C5)** — it loses to Pixen; **parallax (C6)** — worst of the field; **backgrounds (C4)** — it edges PixFlux but not by 12× worth. Characters (C1): a 0.08 margin — marginal upsell only. Note Pro was scored **first-candidate**; best-of-N would lift it further at *small* sizes (many candidates) but **not** for scenes/backgrounds — at ≥256px Pro returns a **single** candidate, so it has no candidate cushion, and still loses C5. Pixen's scene win is therefore robust to the best-of caveat.
...
- **BitForge** is **consistently the weakest on raw quality** (last or near-last in C1/C2/C4/C5). Use it only when its direct `coverage_percentage` subject-fill control is required; never pick it as a default quality play.
...
| **Style-consistent set / reference-driven** | **Pro** (`reference_images`/`style_image`) | Use the dedicated Pro style route; BitForge's lower quality makes it a poor style fallback. |
| **Isolated reusable VFX / effect sprite** | **Pro** (`generate-image-v2`) | Pixen only as an exploratory retry-heavy route; it did not reliably satisfy modularity, subject exclusion, view, and quality together in focused aura testing. |
...
- **VFX is later focused evidence, not an eighth blind category.** The Pro-over-Pixen VFX recommendation comes from the separate [character-aura prompt research spike](pixellab/pixellab-character-aura-prompt-research-spike.md): forty-three Pixen batches plus atlas experiments, compared with repeated successful Pro candidate batches. PixFlux and BitForge were not tested for that target.
