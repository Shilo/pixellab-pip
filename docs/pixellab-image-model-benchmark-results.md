...
- **PixFlux** (`create-image-pixflux` · Aseprite "Create M-XL image") — **standalone objects/props** (ties Pro at 1/12 the cost) and **subject-less backdrops** (best value; beats Pixen). Also a co-best pick for **parallax bands** (tied with Pixen). Aesthetically it leans **painterly / stylized** — a softer, more illustrative render than Pixen's crisp high-detail sprite look — which suits stylized art and looser scenery; when you need strict flat/crisp pixel art instead, drive its `outline` / `shading` / `detail` controls. *(This is a painted **aesthetic**, distinct from style-**reference** / style-transfer work — matching a supplied style image is Pro's domain; see C7.)*
