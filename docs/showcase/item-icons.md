...
Blueprint — replayable route and request body ([`fantasy-rpg-inventory-bgremoved-8x8-32px.blueprint.json`](item-icons/fantasy-rpg-inventory-bgremoved-8x8-32px.blueprint.json)):

```json
[
  {
    "_comment_prompt": "/pixellab-pip create complete 32px inventory item set for fantasy rpg. each item must be unique but consistent style. it must cover all the common items for an rpg game. no background, no border.",
    "_comment": "Showcase winner. Step 2 removes the flat background from step 1's output; the final showcased sheet is the transparent result. Same-canvas 256x256, within the 400x400 remove-background limit.",
  ...
  },
  {
    "POST /v2/remove-background": {
      "image": "01-fantasy-rpg-inventory.png",
      "image_size": {
        "width": 256,
        "height": 256
      },
...
Blueprint — replayable route and request body ([`fantasy-rpg-weapons-8x8-32px.blueprint.json`](item-icons/fantasy-rpg-weapons-8x8-32px.blueprint.json)):

```json
{
  "_comment_prompt": "/pixellab-pip create complete 32px weapon item set for fantasy rpg. each item must be unique but consistent style. it must cover all the common items for an rpg game. no background, no border.",
  "POST /v2/generate-image-v2": {
...
Blueprint — replayable route and request body ([`fantasy-rpg-armor-8x8-32px.blueprint.json`](item-icons/fantasy-rpg-armor-8x8-32px.blueprint.json)):

```json
{
  "_comment_prompt": "/pixellab-pip create complete 32px armor item set for fantasy rpg. each item must be unique but consistent style. it must cover all the common items for an rpg game. no background, no border.",
  "POST /v2/generate-image-v2": {
...
Blueprint — replayable route and request body ([`fantasy-rpg-materials-8x8-32px.blueprint.json`](item-icons/fantasy-rpg-materials-8x8-32px.blueprint.json)):

```json
{
  "_comment_prompt": "/pixellab-pip create complete 32px materials inventory set for fantasy rpg. each item must be unique but consistent style. it must cover all the common items for an rpg game. no background, no border.",
  "POST /v2/generate-image-v2": {
...
Blueprint — replayable route and request body ([`candy-sweets-treats-8x8-32px.blueprint.json`](item-icons/candy-sweets-treats-8x8-32px.blueprint.json)):

```json
{
  "_comment_prompt": "pip create 32px icon set in 8 by 8 sheet that is candy themed with sweets and treats. they must all be individually unique foods. no text, no borders, no backgrounds",
  "POST /v2/generate-image-v2": {
...
Blueprint — replayable route and request body ([`candy-sweets-treats-glossy-8x8-32px.blueprint.json`](item-icons/candy-sweets-treats-glossy-8x8-32px.blueprint.json)):

```json
{
  "_comment_prompt": "pip, create 32px icon set in 8 by 8 sheet that is candy themed with sweets and treats. saturated, shiny, glossy and tasty looking. they must all be individually unique foods. no text, no borders, no backgrounds",
  "POST /v2/generate-image-v2": {
...
Blueprint — replayable route and request body ([`fantasy-rpg-inventory-64px-8x8.blueprint.json`](item-icons/fantasy-rpg-inventory-64px-8x8.blueprint.json)):

```json
{
  "_comment": "From a multi-task 'do the following simultaneously' batch request; see item-icons.md. Single 512x512 atlas (Create Image Pro).",
  "POST /v2/generate-image-v2": {
