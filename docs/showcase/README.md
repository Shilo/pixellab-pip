...
- The generation's replayable `*.blueprint.json` (the exact route plus request body, per [../blueprint.md](../blueprint.md)). Name it after the showcased image it reproduces — `<image-basename>.blueprint.json` — and save it in the page's asset folder beside that image. It is the canonical request-body display: show its contents inline in a fenced code block with a hyperlink to the file, instead of a separate raw request-body block (the blueprint already contains the body). Use a bundle array only when the result took more than one PixelLab call, such as a generation followed by a background-removal or animation step; local post-processing (palette-clamp recolors, cropping, sheet assembly) is not a blueprint step — record it under local processing notes instead. A derived composite that only re-arranges an already-blueprinted generation (e.g. a side-by-side palette comparison) can point to that generation's blueprint rather than getting its own.
