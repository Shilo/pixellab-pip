...
The standing column reuses the reviewed static rotation. The two movement columns come from the four returned walk frames, which means these engines ship only half of what was generated: `walking-4-frames` returns four frames per direction, the three-column layout delivers two, and the remaining two are discarded. XP is the only supported engine that uses all four.

Which two survive is decided per direction by inspection, not by a fixed index. The blueprint asks for "a visually verified, chronologically ordered pair of distinct opposing strides" and deliberately names no frame number, because PixelLab documents the template's frame count but not which gait phase lands on each returned index. In practice all three runs converged on the same answer — every one of the twelve direction-rows selected frames `02` and `04` and dropped `01` and `03` — but that is an observed outcome, not a constant to rely on.

The selection was measured as well as eyeballed. The two frames of each pair differ by 598 to 753 pixels in MZ, 529 to 587 in MV, and 250 to 281 in the smaller VX Ace sprite, so no movement column is a duplicate of the other or a near-repeat of standing. The stride itself shows up as horizontal foot-centroid displacement: 4.1 to 6.9 pixels between the two chosen frames in the 48px runs, against 0.2 to 1.9 pixels for the rejected pair, whose weight never crosses sides.

That margin is a function of sprite size, and VX Ace shows where it runs out. At 32px the same displacement compresses to 0.9 to 3.3 pixels, and in the north row the rejected pair actually scored marginally higher. The pair still reads correctly by eye, but the numbers alone would not have settled it — which is why the blueprint requires visual verification instead of a measured threshold.
